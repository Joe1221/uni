...
\begin{st}[Korselt, 1899] \label{3.8}
	...
	\begin{proof}
		\begin{enumerate}[1.]
			...
			\item
				Ist $n$ eine Carmichaelzahl, dann $\phi(n) \divs n-1$, also $p-1$ teilt $n-1$, wenn $p$ ein Teiler von $n$ ist.
				Angenommen $n = pq$ und $p-1$ teile $n-1 = pq - 1$ und $q -1$ teile ebenfalls $pq - 1$.
				$p-1$ teilt $pq-1$ und $(p-1)q = pq - q$, also $p-1$ teilt $pq-1 - pq + q = q -1$ und somit $p-1$ teilt $q-1$.
				Analog sieht man $q-1$ teilt $p-1$ und damit $p = q$, ein Widerspruch.
			\item
