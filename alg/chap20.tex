...
Permutationen $\sigma \in O_n$ induzieren $K$-Automorphismen von $R$, bzw. $E$ wie folgt.

\begin{df} \label{20.4-2}
	Sei $\sigma \in O_n$.
	Dann wird durch $\sigma(t_i) = t_{\sigma(i)}$ ein $K$-Algebraautomorphismus von $R$ und von $E$ induziert (universelle Eigenschaft von Polynomringen \ref{15.1-17} und Fortsetzung auf Quotientenkörper \ref{15.2-4}).
	Offensichtlich ist $O_n$ auf diese Weine (endliche) Untergruppe von $\Aut_K(E) = G(E / K)$.
\end{df}

\begin{st} \label{20.4-3}
	Sei $K$ Körper, $E = K(t_1, \dotsc, t_n)$ und $O_n \le \Aut_K(E)$, wie in \ref{20.4-2}.
	Sei $L = E^{O_n}$ der Fixkörper von $E$ unter $O_n$.
	Dann ist $E$ algebraisch und galoissch über $L$ mit Galoisgruppe $O_n = G(E / L)$.
	\begin{proof}
		Klar mit \ref{20.4-1}.
	\end{proof}
\end{st}

Wir wollen jetzt den Fixkörper $L$ unter $O_n$ näher beschreiben:
Eine rationale Funktion (Polynom) $f(t_1, \dotsc, t_n) \in E$ (bzw. $R$) heißt \emphdef{symmetrisch}, falls $\sigma f = f$ ist für alle $\sigma \in O_n$ (d.h. falls $f \in L$ ist).
So ist
\[
	f(t_1, \dotsc, t_n) \in L
	\iff
	\forall \sigma \in O_n : f(t_1, \dotsc, t_n) = f(t_{\sigma(1)}, \dotsc, t_{\sigma(n)}).
\]
...
\begin{ex} \label{20.4-4}
	...
	\end{align*}
	Offensichtlich erzeugen $s_1, \dotsc, s_n$ über $K$ eine $K$-Unteralgebra $K[s_1, \dotsc, s_n]$ von $K[t_1, \dotsc, t_n]$ und es gilt $K[s_1, \dotsc, s_n] \subset L = E^{O_n}$.
\end{ex}
...
\begin{kor} \label{20.4-6}
	...
	Dann sind die elementarsymmetrischen Funktionen $s_1, \dotsc, s_n$ in den $t_i$ algebraisch unabhängig und erzeugen einen Unterkörper $K(s_1, \dotsc, s_n)$ von $E$, der in $L = E^{O_n}$ enthalten ist: $K \subset K(s_1, \dotsc, s_n) \subset L \subset E$.
\end{kor}
...
\begin{kor} \label{20.4-8}
	...
	Insbesondere ist $E$ algebraisch und galoissch über $K(s_1, \dotsc, s_n)$ mit Galoisgrupppe $O_n$.
	\begin{proof}
		Nach \ref{19.5-1} ist $G(E / L_1) \le O_n$ (als Permutationen der Nullstellen von $f$), $L_1 = K(s_1, \dotsc, s_n)$.
		Wir wissen bereits $G(E / L) = O_n$.
		...
		\[
			...
			\le O_n
		\]
		Also ist $G(E / L) = G(E / L_1) = O_n$ und nach dem Hauptsatz $L = L_1$.
	\end{proof}
\end{kor}
...
\begin{lem} \label{20.4-9}
	...
	\begin{proof}
		...
		\[
			H(t_1, \dotsc, t_n)
			:= \prod_{\sigma \in O_n} h\Big(t_{\sigma(1)}, \dotsc, t_{\sigma(n)} \Big)
			= \prod_{\sigma \in O_n} \sigma h .
		\]
		Dann ist $\tau H = H$ für alle $i \in O_n$.
		...
		Beachte, dass $H(t_1, \dotsc, t_n) = h(t_1, \dotsc, t_n) \prod_{1 \neq \sigma \in O_n} h(t_{\sigma(1)}, \dotsc, t_{\sigma(n)})$.
		Also ist
		\[
			H(\alpha_1, \dotsc \alpha_n)
			= h(\alpha_1, \dotsc, \alpha_n) \prod_{1 \neq \sigma \in O_n} \sigma h(\alpha_1, \dotsc, \alpha_n)
			= 0.
		\]
		Andererseits wegen $s_i(\alpha_1, \dotsc, \alpha_n) = \pm a_i$ ist $\eps(N(s_1, \dotsc, s_n)) = N(\pm a_{n-1}, \dotsc, \pm a_0) = \tilde N(a_0, \dotsc, a_{n-1}) = 0$.
...
\begin{kor} \label{20.4-10}
	Sei $K$ ein Körper, $F = K(a_0, \dotsc, a_{n-1})$ Körper der rationalen Funktionen in den Variablen $a_0, \dotsc, a_{n-1}, f = a_0 + a_1 x + \dotsb + a_{n-1} x^{n-1} + x^n \in F[x]$ und sei $E$ der Zerfällungskörper von $f$ über $F$.
	Dann ist $E$ galoissch über $F$ mit Galoisgruppe $G(E / F) \isomorphic O_n$.
	\begin{proof}
		 ...
		 Dabei gehen die elmentarsymmetrischen Funktionen $s_i(t_1, \dotsc, t_n$ bis aufs Vorzeichen in $\eps(s_i) = s_i(\alpha_1, \dotsc, \alpha_n) = \pm a_{n-i} \in F$ über und daher auch der Fixkörper $K(s_1, \dotsc, s_n)^L = K(t_1, \dotsc, t_n)^{O_n}$ in den Körper $f = K(a_0, \dotsc, a_{n-1})$.
		 Also haben wir $K$-Isomorphismen $\eps: K(t_1, \dotsc, t_n) \to K(\alpha_1, \dotsc, \alpha_n) = E$ mit $\eps(L) = \eps(K(t_1, \dotsc, t_n)^{O_n}) = F$.
		 Also ist $\Aut_F(E) = G(E / F) \isomorphic \Aut_L (K(t_1, \dotsc, t_n)) = O_n$ (siehe \ref{20.4-8}).
	\end{proof}
\end{kor}

\begin{nt} \label{20.4-11}
	Wegen $|O_n| = n!$ folgt unmittelbar
	\begin{enumerate}[1.]
		\item
			Da $\deg f = n$ ist, ist $f$ irreduzibel und separabel über $F$.
		\item
			Seien $\alpha_1, \dotsc, \alpha_n$ die Nullstellen von $f$ in $E$, $L_i = F(\alpha_1, \dotsc, \alpha_i) \subset E$ für $i = 1, \dotsc, n$.
			Dann ist $L_0 = F \subset L_1 \subset \dotsb \subset L_n = E_1$, $L_i = L_{i-1}(\alpha_i)$ mit $\mu_{\alpha_i, L_{i-1}} = (x-\alpha_i)\dotsb (x-\alpha_n) \in L_{i-1}[x]$ für $i = 1, \dotsc, n-1$.
	\end{enumerate}
	\begin{proof}
		...
		Wir haben $[L_1 : L_0] \le n$, da $\mu_{\alpha_1, F} \divs f$ ist.
		$[L_2 : L_1] \le n-1$, da $\mu_{\alpha_2, L_1} \divs (x-\alpha_2) \dotsb (x-\alpha_n)$ usw.
