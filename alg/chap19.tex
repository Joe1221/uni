...
\begin{df} \label{19.1-1}
	Ein Polynom $f \in K[x]$ heißt \emphdef[separabel]{separabel über $K$}, falls $f$ keine mehrfachen Nullstellen in $\_K$ besitzt, d.h. $f = \lambda (x - \alpha_1) \dotsc (x - \alpha_n) \in \_K[x]$ mit $\alpha_1, \dotsc, \alpha_n \in \_K$ paarweise verschieden, $\lambda \in \_K$, $n = \deg f$.
	\begin{note}
...
\begin{df} \label{19.1-10}
	...
	Der \emphdef[Separabilitätsgrad]{Separabilitätsgrad $[E : K]_S$ von $E$ über $K$} ist definiert als $|\Hom_{K}(E, \_K)|$, also die Anzahl der verschiedenen $K$-linearen Körperhomomorphismen von $E$ nach $\_K$.
\end{df}
...
\begin{st} \label{19.2-3}
	...
	%\begin{proof}
	%	Sei $\_\F_p = E$ der algebraische Abschluss von $\F_p$.
	%	Nun gilt für $\alpha \in \_\F_q$ die Gleichung $\alpha^{q-1} = 1$ genau dann, wenn $\alpha^q - \alpha = 0$ ist, d.h. wenn $\alpha$ Nullstelle von $x^q - x \in \F_p[x]$ ist.
	%	Weiter ist $\ddx (x^q - x) = qx^{q-1} - 1 = -1 \neq 0$ und daher ist $x^q - x$ separabel über $\F_p$.
	%	Also zerfällt $x^q - x$ in Linearfaktoren $\prod_{i=0}^{q-1} (x-\alpha_i)$ mit $\alpha_0 = 0, \alpha_1, \dotsc, \alpha_{q-1} \in \_{\F_p}$.
	%	So gilt $\alpha_i^q = \alpha_i$ für $i \in \Set{0, \dotsc, q-1}$.
	%\end{proof}
\end{st}
...
\begin{df} \label{19.4-2}
	Eine algebraische, separable und normale Körpererweiterung $E$ von $K$ heißt \emphdef[Galoiserweiterung]{Galoiserweiterung von $K$} oder \emphdef[galoissch]{galoissch über $K$}.
\end{df}
...
\begin{df} \label{19.6-1}
	Sei $\Char K = p > 0$.
	Dann heißt $\alpha \in \_K$ \emphdef{rein inseparabel} über $K$, falls $\alpha^{p^n} \in K$ ist für ein $n \in \N$.
	\begin{note}
