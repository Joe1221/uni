...
\section{Primkörper und Charakteristik}
...
\begin{note}
	...
	Ist $S \subset K = K(S)$ und $\hat S = \{ \alpha \in S : \{\alpha \not\in K\}$, so ist $E = K(\hat S) = K(S)$.
...
Der Trick von \ref{sec:2} geht nochmals, nun mit $K[x]$ (dies ist ein Hauptidealring) anstatt $\Z$ und der universellen Eigenschaft von $K[x]$:
...
\begin{lem} \label{18.3-1}
	...
	In diesem Fall ist das Polynom $p \in K[x]$ irreduzibel und bis auf Skalara das eindeutig bestimmte Polynom kleinsten Grades mit Nullstelle $\alpha$ ($p(\alpha) = 0$).
\end{lem}
...
\begin{df} \label{18.3-2}
	...
	\begin{enumerate}[i)]
		\item
			...
			Wegen $\dim_K K[x] = \infty$ sind $\dim_K K(x)$ und daher auch $\dim_K K(\alpha)$ und $\dim_K E$ unenlich.
		...
		\item
			...
			So ist
			\[
				K(\alpha)0= \big\{ f(\alpha) : f \in K[x], \deg f \le k - 1 \big\}.
			\]
	\end{enumerate}
\end{df}
...
\begin{conv*}
	Im folgenden wird der algebraische Abschluss eines Körpers $K$ mit $\_K$ beszeichnet.
...
\begin{nt} \label{18.4-10}
	\begin{enumerate}[1.)]
		...
		\item
			...
			Insbesondere ist der algebraische Abschluss $\_Q$ von $\Q$ abzählbar und daher $\_\Q \subsetneq \C$.
