				...
				Daraus folgt
					\begin{align*}
						...
						\intertext{Wähle $q \in ]1,\infty[$ mit $\f 1q + \f 1p = 1$ (oder äquivalent $\f pq = p-1$) und wende die Höldersche Ungleichung an:}
							...
							+ \underbrace{\bigg( \int_{\Omega} |g|^p \dx[\my] \bigg)^{\f 1p}}_{N_p(g)} \underbrace{\bigg( \int_{\Omega} |f+g|^{q(p-1)} \dx[\my] \bigg)^{\f 1q} }_{N_p(f+g)^{\f pq}}.
						\end{align*}
				...
				\begin{enumerate}[a)]
					\item
						...
						\begin{align*}
							...
							&= \lim_{j\to \infty} \int_{\Omega} \underbrace{|f-s_j|^p}_{\le |f|^p} \dx[\my],  \qquad \int_{\Omega} |f|^p \dx[\my] < \infty\\
...
%			\end{note}
		...
		\item
			...
			\]
			wird erzeugt vom Skalarprodukt $\<f,g\> := \int_{\Omega} f \_g \dx[\my]$ (Wegen Hölder: $\int_{\Omega} |f \_g|\dx[\my] \le \|f\|_2 \|g\|_2 < \infty$, konvergiert das Integral)

			Also ist $L^2$ ein Hilbertraum.
	\end{enumerate}
	\begin{proof}[Übung]
	\end{proof}
        \begin{note}[zu \ref{2.16} 4)]
	Mit der Hölder-Ungleichung folgt die Wohldefiniertheit des Skalarprodukts. $L^2$ ist insbesondere nach \ref{2.12} vollständig.
	\end{note}
\end{st}
