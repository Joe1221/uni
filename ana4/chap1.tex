...
\begin{ex} \label{1.6}
	\begin{enumerate}[1)]
		\item
			...
			$(L, \<\argdot,\argdot\>)$ ist ein Hilbertraum (Beweis: Übungsaufgabe \coursehref{blatt01.pdf}{1.2a}).
		...
		\item
			...
			\]
			Für die Gültigkeit von (S1) bis (S3) aus \ref{1.1}, siehe Übungsaufgabe \coursehref{blatt01.pdf}{1.3a}.

			Allerdings ist $(L,\<\argdot,\argdot\>)$ kein Hilbertraum (Gegenbeispiel Übungsaufgabe \coursehref{blatt01.pdf}{1.3b}).
...
\begin{ex} \label{1.13}
	Sei $L = C([-\pi,\pi] \to \C)$ mit Skalarprodukt (siehe Übungsaufgabe \coursehref{blatt01.pdf}{1.3a})
	\[
		\<f,g\> := \int_{-\pi}^{\pi}f(x)\_{g(x)} \dx
	\]
	und Orthonormalsystem (siehe Übungsaufgabe \coursehref{blatt01.pdf}{2.1a})
	\[
...
\begin{st} \label{1.16}
	...
	\begin{proof}
		Übungsaufgabe \coursehref{blatt01.pdf}{1.2b}
	\end{proof}
\end{st}
		...
		\begin{enumerate}[1)]
			...
			\item
				\textbf{$(g_j)$ konvergiert gleichmäßig auf $K$:}
					...
					\[
						\forall \eps > 0 \exists J \in \N \forall j,k \in K : |g_j(x) - g_k(x)| < \eps
					\]
					...
					\begin{enumerate}[a)]
						\item
							Wähle $\{\xi_{l_1}, \dotsc, \xi_{l_N} \}\subset M$ so dass $K \subset \bigcup_{m=1}^N K_\delta (\xi_{l_m})$ (funktioniert, da $K$ (überdeckungs-)kompakt, siehe Konstruktion von $M$).
						\item
