			...
			\item[4)]
				\begin{enumerate}[a)]
					\item
						...
						Zu jeder offenen Menge $O \subset \R^{n+m}$ mit $(x,y) \in O$ existiert $\phi \in C_0^\infty(O)$ mit $T \times S(\tilde \phi) \neq 0$.
						...
						\end{align*}
						und damit als Fortsetzung durch $O$ auf $O \setminus (O_1\times O_2)$
						\[
					...
					\item
						...
						Es gilt
						\[
							\exists O_1, x \in O_1 \subset \R^n \forall \phi_1 \in C_0^\infty(O_1)
							: T(\tilde \phi_1) = 0
						\]
						Setze $O := O_1 \times \R^m \ni (x,y)$.
						Für $\Phi \in C_0^\infty(O)$ gilt $\Phi(\argdot, y) \in C_0^\infty(O_1)$ für $y \in \R^m$.
						Also
						\[
...
Die Idee ist jetzt:
definiere $f \ast g$ nur für solche $(f,g)$, für die gilt
\begin{align*}
	&\forall z \in \R^n : \{y \in \R^n : f(z-y) g(y) \neq 0 \} \text{ ist beschränkt} \\
	\iff &\forall z \in \R^n : \{(x,y) : f(x) \neq 0 \land g(y) \neq 0 \land x + y = z\} \text{ ist beschränkt} \\
	\iff &\forall z \in \R^n : ((\supp f) \times (\supp g)) \cap \{(x,y) : x+y = z \} \text{ ist beschränkt}
\end{align*}

\begin{df} \label{5.35}
	Seien $T, S \in \scr D'(\R^n)$, $\sigma_a := \{(x,y) : |x+y| \le a \}$.
...
\begin{nt} \label{5.37}
	...
	\begin{align*}
		...
		&= \int_{\R^n} \int_{\R^m} f(x-y)g(y) \dx[y] \phi(x) \dx \\
		&\stack[{\dx[z]=\dx}]{z:=x-y} = \int_{y\in \R^n} \int_{z\in \R^n} f(z) \phi(z+y) g(y) \dx[z] \dx[y] \\
		&= \int_{z\in \R^n} f(z) \int_{y\in \R^n} g(y) \phi(z+y) \dx[y] \dx[z] \\
		&= T_{f,z} \big( T_{g,y} (\phi(z+y)) \big) \\
		&= (T_f \times T_g) (\Phi) \qquad (\Phi(z,y) := \phi(z+y)).
	\end{align*}
	Ein Problem beim Verwenden dieser Darstellung ist, dass $\supp \psi$ stets nicht beschränkt ist: $\psi \in C^\infty(\R^n) \setminus C_0^\infty(\R^n)$.
\end{nt}
...
\begin{st}[Definition und Satz] \label{5.38}
	...
	\begin{proof}
		\begin{enumerate}[1)]
			\item
				Sei $\tilde \psi_\phi \in C_0^\infty(\R^{2n})$, $\tilde \phi = 1$ auf $(\supp T \times S) \cap \sigma_5$.
				Dann ist
				\begin{align*}
			...
			\item
				\begin{enumerate}[a)]
					...
					\item
						Sei $\phi_n \stack{\scr D(\R^n)} \phi$, d.h. $\forall j \in \N: \supp \phi_j \subset K$ und $\forall \alpha \in \N_0^n : \nabla^\alpha \phi_j \to \nabla^\alpha \phi$ gleichmäßig in $\R^n$.
						...
						\begin{enumerate}[(i)]
							...
							\item
								$\nabla^{(\alpha,\beta)} (\psi_\phi \Phi_j) \to \nabla^{(\alpha, \beta)} (\psi_\phi \Phi)$ gleichmäßig auf $\R^{2n}$, denn
								\[
						...
						\end{enumerate}
						Also $\psi_\phi \Phi_j \stack{\scr D(\R^{n+m})}\longrightarrow$ und damit da $T \times S$ stetig:
						\[
							T \ast S(\phi_j)
	...
	\begin{enumerate}[1)]
		\item
			...
			\begin{enumerate}[a)]
				...
				\item
					Sei $\phi \in C_0^\infty(\R^n)$, $\supp \subset K_R(0)$.
					Setze $\Phi(x,y) := \phi(x+y)$, dann ist $\supp \Phi \subset \sigma_R$.
					Definiere
					\[
						(\underbrace{\R^n}_{\supset \supp T} \times \underbrace{\{0\}}_{= \supp \delta_0}) \cap \sigma_R
						=: K,
					\]
					$\psi_\phi \in C_0^\infty(\R^{2n})$, $\psi_\phi = 1$ auf $K$.
					Dann gilt
					\begin{align*}
						T \ast \delta_0(\phi)
						= T \times \delta_0 (\psi_\phi \Phi)
						= T_x (\underbrace{\delta_{0,y}(\psi_\phi(x,y) \phi(x+y))}_{=\psi (x,0) \phi(x+0) = 1 \phi(x)})
						= T(\phi).
					\end{align*}
