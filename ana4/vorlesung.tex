	...
	\begin{proof}[Beweisskizze]
		...
		\begin{enumerate}[1)]
			\item
				...
				Dann ist
				\[
					|Af_j(x)| \le \underbrace{\int_{K} |G(x,y) |f(y)| dy}_{\le\<c,|f_j|\>} \stackrel{CSB}\le \|c\| \|f_j\| \le \tilde c
				\]
			\item
		...
		\begin{enumerate}[1)]
			...
			\item
				$(g_j)$ konvergiert gleichmäßig auf $K$
					...
					\end{enumerate}
					So finden wir für jedes $ x $ ein $ \sigma_{l_i}, i\in\{1,...,N\} $, sodass $ x $ in der zugehörigen $\delta $-Umgebung enthalten ist.
					\begin{align*}
						|g_j(x) - g_k(x)| 
						&\le \underbrace{\Big|g_j(x)-g_j(\xi_l)\Big|}_{\text{$< \eps$ falls $x-\xi_{l_i}|<\delta$}} 
						+ \underbrace{\Big|g_j(\xi_{l_i}) - g_k(\xi_{l_i})\Big|}_{\text{$<\eps$ falls $j,k > J$, $J$ abhängig von $\xi_{l_i}$}}  + \underbrace{\Big| g_k(\xi_{l_i}) - g_k(x)\Big|}_{\text{$<\eps$ falls $x-\xi_{l_i} < \delta$, $\delta$ unabhängig von $k$ bzw. $j$}} 
					\end{align*}
