	...
	\begin{enumerate}[1)]
		...
		\item
			...
			\begin{align*}
				...
				\tilde A f &:= \sum_{i=1}^\infty j^2 \<f, \sqrt{\f 2\pi}\sin(j\cdot) \> \sqrt{2\pi}\sin(j\cdot).
			\end{align*}
			Dann gelten
			\begin{itemize}
				\item
...
\begin{nt} \label{1.22}
	Es liege $A$ in der Spektraldarstellung aus \ref{1.19} vor.
	Dann ist
	\[
...
\begin{df} \label{1.23}
	...
	\begin{align*}
		...
		&= \sup_{\|x\|\neq 0} \f{\|Ax\|}{\|x\|}
	\end{align*}
	(Beweis: Einfache Übung zum Supremum, linearer Operator, Norm)
...
\begin{nt} \label{1.24}
	$A$ besitze die Spektraldarstellung aus \ref{1.19}.
	Dann gilt
	\[
	...
	\begin{proof}
		...
		\begin{enumerate}[1)]
			...
			\item
				...
				\begin{align*}
					|Af_j(x) - Af_j(\tilde x)|
					&= \underbrace{\bigg| \int_K \Big(\underbrace{G(x,y) - G(\tilde x, y)}_{|\cdot|< \eps}\Big) f_j(y) dy \bigg|}_{\le \<\eps, |f_j|\> = \int_K \eps |f_j(y)| dy} \\
					&\stack{CSB}\le \|\eps\| \|f_j\| \\
					&= \sqrt{N(K)} \eps \|f_j\|  \qquad \|\eps\| = \sqrt{\int_{K}|\eps|^2 dx}
					&\le c\eps
				\end{align*}
			\item
