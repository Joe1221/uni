...
\begin{ex} \label{1.6}
	\begin{enumerate}[1)]
		...
		\item
			...
			$(L,\<\cdot,\cdot\>)$ ist kein Hilbertraum (Gegenbeispiel Übung).
			...
			\end{align*}
			$(\tilde L, \<\cdot,\cdot\>^\sim)$ ist ein Hilbertraum.
...
\begin{st} \label{1.8}
	...
	\begin{itemize}
		\item
			$L \subset H$;
		\item
			$\<\cdot,\cdot\>_H = \<\cdot,\cdot\>_L$ auf $L\times L$;
		\item
			$L$ ist dicht in $H$.
	\end{itemize}
...
\begin{df} \label{1.9}
	Eine Folge $(e_j)$ im Prähilbertraum $(L,\<\cdot,\cdot\>)$ heißt \emph{Orthonormalsystem}, falls
	\[
		\<e_j,e_k\> = \delta_{j,k}.
	\]
...
\begin{st} \label{1.12}
	Sei $(e_j)$ ein Orthonormalsystem in $(L,\<\cdot,\cdot\>)$ und $x\in L$.
	Dann gilt:
	\begin{enumerate}[1)]
		\item
		...
		\item
			Ist $(L, \<\cdot,\cdot\>)$ zusätzlich Hilbertraum, so ist $\sum_{j=1}^\infty \<x,e_j\> e_j$ konvergent (aber nicht unbedingt gegen $x$).
	\end{enumerate}
	\begin{proof}
		\begin{enumerate}[1)]
...
\begin{df} \label{1.14}
	Sei $(L, \<\cdot,\cdot\>)$ ein Prähilbertraum und $(e_j)$ ein Orthonormalsystem in $L$.
	\begin{enumerate}[1)]
		\item
			Für $x\in L$ heißt
			\[
...
\begin{st} \label{1.16}
	...
	Ist $(H,\<\cdot, \cdot\>)$ Hilbertraum mit VONS $(e_j)$ so ist
	\begin{align*}
		\Phi: H &\to \ell^2 \\
...
\begin{nt} \label{3.4}
	Ist zusätzlich zu \ref{3.3} $(L,\<\cdot,\cdot\>)$ ein Hilbertraum, dann kann 2) in der Form
	\[
...
\begin{st}[Hauptsatz über symmetrische, kompakte Operatoren] \label{3.5}
	Sei $(L, \<\cdot,\cdot\>)$ unendlichdimensionaler Prähilbertraum, $A: L \to L$ linear, symmetrisch und kompakt.
