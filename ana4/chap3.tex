...
\begin{st} \label{3.2}
	...
	\end{enumerate}
	\begin{proof}
		Übungsaufgabe \href{http://www.iadm.uni-stuttgart.de/LstAnaMPhy/Lesky/Vorlesungen/13-Hoehere-Analysis/blatt07.pdf}{7.2}
	\end{proof}
\end{st}
...
\begin{nt} \label{3.4}
	...
	\]
	\begin{proof}
		 Sei $x\in L$ beliebig. Dann folgt aus dem Projektionssatz (aus der Funktionalanalysis, siehe Übungsaufgabe \href{http://www.iadm.uni-stuttgart.de/LstAnaMPhy/Lesky/Vorlesungen/13-Hoehere-Analysis/blatt07.pdf}{7.1}) eine eindeutige Zerlegung 
\[
...
\begin{st}[Satz von Sturm-Liouville] \label{3.19}
	...
	\begin{enumerate}[1)]
		...
		\item
			Die Eigenfunktion $u_j$ (jeweils zum Eigenwert $\lambda_j$) können so normiert werden, dass $(\sqrt {r} u_j)$ ein ONS in $L = C(I \to \R)$ bilden.
...
\begin{lem} \label{3.24}
	...
	\begin{proof}
		Nach Satz \ref{4.18} ist $C_0^\infty$ dicht in $L^p(\R)$ (später). \fixme[Beweis vervollständigen]. 
	\end{proof}
\end{lem}
