...
\begin{st} \label{3.2}
	...
	\end{enumerate}
	\begin{proof}
		Siehe \coursehref{blatt07.pdf}{Übungsaufgabe 7.2}.
	\end{proof}
\end{st}
...
\begin{nt} \label{3.4}
	...
	\]
	\begin{proof}
		 Sei $x\in L$ beliebig. Dann existiert nach dem Projektionssatz (aus der Funktionalanalysis, siehe \coursehref{blatt07.pdf}{Übungsaufgabe 7.1}) eine eindeutige Zerlegung
\[
...
\coursetimestamp{6}{5}{2013}
			...
			\item
				...
				Für eine beschränkte Folge $(x_n)$ in $\{v_1\}^\orth$ existiert eine konvergente Teilfolge bezüglich $A$:
				\[
					A x_{n_k} \to y \in L.
				\]
				Es gilt $A_1 x_{n_k} \in \{v_1\}^\orth$ und damit $y \in \{v_1\}^\orth$, da $\{v_1\}^\orth$ abgeschlossen.
				Also ist $A_1$ kompakt.
				...
				\begin{enumerate}[1. {Fall}]
					...
					\item
						...
						Ein Widerspruch zu $A$ kompakt, also gilt $\lambda_j \to 0$.
				\end{enumerate}
			\item
...
\begin{ex}[Schwingende Saite] %\label{3.5}
	...
	\[
		% FIXME: Das sollte vlt mal jemand nachrechnen
		...
		\begin{seg}[(ii)$\implies$(i)]
			...
			\begin{itemize}
				...
				\item
					...
					\]
					% FIXME: Hier fehlt doch eine Zusatzbedingung, damit $W(x) \neq 0$? Nur die andere Richtung gilt ohne weiteres.
...
\begin{st}[Erweitertes Kriterium von Dini] \label{3.29}
	...
	\begin{proof}
		...
		nach \ref{3.25}, da
		\begin{align*}
			g_1(t) &= \Ind_{[-\pi,0]} \cdot \f 1{2\pi} \underbrace{\f {f(t+x) - f(x-0)}t}_{\in L^1([-\pi,0])}  \f {t}{\sin (\f 12 t)}
				\quad \implies g_1 \in L^1([-\pi,\pi]), \\
			g_2(t) &= \Ind_{[0,\pi]} \cdot \f 1{2\pi} \underbrace{\f {f(t+x) - f(x+0)}t}_{\in L^1([0,\pi])}  \f {t}{\sin (\f 12 t)}
					...
					\item
						...
						\begin{align*}
							...
							&= \Bigg| \int\limits_{[-\pi,-\delta] \cup [\delta, \pi]} \Big( f(x+t) - f(x'+t) \Big) D_n(t) \dx[t]  \\
								...
								&\quad\; - \int\limits_{[-\pi,-\delta] \cup [\delta,\pi]} \Big(f(x) - f(x')\Big) D_n(t) \dx[t]
									+ \underbrace{\int_{-\pi}^\pi \Big(f(x) - f(x')\Big) D_n(t) \dx[t]}_{= f(x)-f(x')}
									\Bigg| \displaybreak[0] \\
							&\le \int\limits_{[-\pi,-\delta]\cup [\delta, \pi]}  \Big(\underbrace{|f(x+t) - f(x'+t)| + | f(x) -f(x')|}_{\le c |x-x'|^\alpha}\Big) \underbrace{|D_n(t)|}_{\le \f 1{2\pi \sin(\f \delta 2)}} \dx[t] \\
								&\quad + \int_{-\delta}^\delta \Big(\underbrace{|f(x+t) - f(x)| + |f(x'+t)-f(x')|}_{\le 2c|t|^\alpha}\Big) \underbrace{|D_n(t)|}_{\le \f 1{2\pi|\sin(\f t2)|}} \dx[t] \\
								&\quad + \underbrace{|f(x) - f(x')|}_{\le 2c |x-x'|^\alpha} \displaybreak[0]\\
							&\le \underbrace{c \bigg( 1 + \int\limits_{[-\pi,-\delta] \cup [\delta, \pi]} \f 1{\pi \sin(\f \delta 2)} \dx[t] \bigg)}_{=:c(\delta)} |x-x'|^\alpha
								+ \f c\pi \int_{-\delta}^\delta \f {|t|^\alpha}{|\sin(\f t2)|} \dx[t].
						\end{align*}
						Da $\int_{-1}^1 \f {|t|^\alpha}{|\sin \f t2|} \dx[t] < \infty$, wählen wir $\delta > 0$ so, dass
						\[
						   \f c\pi \int_{-\delta}^\delta \f {|t|^\alpha}{|\sin (\f t2)|} \dx[t] < \eps.
						\]
						Wähle anschließend $\delta' > 0$ so, dass $c(\delta)|x-x'|^\alpha < \eps$ für $|x-x'|<\delta'$.
						Dann folgt
						\[
						    |s_n(x)-s_n(x')|
						    \le \dotsb
						    \le \underbrace{c(\delta)|x-x'|^\alpha}_{<\eps} + \underbrace{\f c\pi \int_{-\delta}^\delta \f {|t|^\alpha}{|\sin(\f t2)|}}_{<\eps} \dx[t]
						    < 2\eps
						    \qquad \text{für $|x-x'| < \delta'$}
						\]
...
\begin{nt}[Komplexe Darstellung von Fourierreihen] \label{3.32}
...
\begin{nt}[Andere Intervalle] \label{3.35}
	...
	% FIXME: Muss klarer gemacht werden: Warum ist die Lipschitz-Bedingung nötig? Wie genau übertragen sich die Aussagen auf das neue VONS (in den Beweisen wurde bislang mit s_n(x) gearbeitet)?
\end{nt}
