...
\begin{proposition}[Homotopie-Invarianz] \label{thm:prop:wn_hominv}
...
Die Menge $V_\Gamma$ ist im idealen Fall diskret, wie in Abbildung \ref{fig:polyiseczeros} kann jedoch auch nicht-isolierte Punkte enthalten, wenn beispielsweise $q$ und $r$ gleiche Faktoren besitzen.
...
\begin{definition} \label{thm:def:gamma2_eps}
    ...
    \end{math}
    wobei $B_\epsilon(x,y)$ den Ball um $(x,y)$ mit Radius $\epsilon$ in der Supremumsnorm bezeichnet ($B_\epsilon(x,y)$ ist also selbst ein Rechteck).
\end{definition}
...
\begin{lemma} \label{thm:lem:wn1_const}
    ...
    \begin{proof}
        ...
        Die Aussage folgt dann mittels \ref{thm:lem:wn1_null}.
        ...
        Für alle $0 < \epsilon < \epsilon_0$ lassen sich die Rechtecke $\Gamma_{\epsilon_0}$ und $\Gamma_\epsilon$ über eine stückweise polynomielle Homotopie ineinander überführen.
        Die Aussage folgt dann aus \ref{thm:prop:wn_hominv}.
    \end{proof}
\end{lemma}

Wenn im Folgenden von $\deg(q,r|\Boundary\Gamma_\epsilon(x,y))$ die Rede ist, so sei $\epsilon$ stets gemäß \ref{thm:lem:wn1_const} hinreichend klein gewählt.

\begin{definition} \label{thm:def:idx}
    Sei $(\Gamma, p, q, r)$ zulässig.
    Wir definieren
    \begin{math}
        \Index{\Gamma_\epsilon(x,y)}{p}{q,r} &:= \sgn(p) \deg(q,r|\Boundary\Gamma_\eps), \\
...
\begin{proposition}[Eigenschaften] \label{thm:prop:idx_prop}
    ...
    \begin{enumerate}[i)]
        \item
            \emph{Subdivision}: Falls $\_V_{\Gamma_1} \cup \_V_{\Gamma_2} = \_V_\Gamma$ und $\Gamma_1 \cap \Gamma_2 \cap \_V_{\Gamma} = \emptyset$ gilt
            \begin{math}
    ...
    \begin{proof}
        \begin{enumerate}[i)]
            \item
                Zunächst besitzen dank Voraussetzung $\Boundary\Gamma_1$ und $\Boundary\Gamma_2$ keine Punkte aus $\_V_{\Gamma}$ und somit sind $(\Gamma_1, p, q, r)$ und $(\Gamma_2, p, q, r)$ zulässig.
            ...
            \item
                ...
                Für alle $(x,y) \in V(q,r)$ ist $\sgn(s)$ konstant und
                \begin{math}
            ...
            \item
                ...
                Die Aussage folgt durch Anwenden von \ref{thm:lem:wn1_rot} in jedem Punkt $(x,y) \in V(q,r)$.
            \item
            ...
            \item
                Sei $(x,y) \in V(q,r)$, dann gilt per definitionem
                \begin{math}
...
\begin{note}
    Im Zuge dieser Bachelorarbeit ist eine teilweise Implementierung des Algorithmus entstanden \footnote{Quellcode erhältlich auf \url{http://www.stud.mathematik.uni-stuttgart.de/~hilbsn/bth/code}}.

    Für $z = x + yi$ wurden aus der Darstellung $z^n = p + iq$ einfache Beispiele ermittelt ($p + iq$ beschreibt eine $n$-fache Wicklung um die Null) für die erfolgreich der Kronecker-Index bestimmt werden konnte.
