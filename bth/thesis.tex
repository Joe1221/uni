...
\begin{titlepage}
    \begin{center}
        ...
        \begin{tikzpicture}[
            scale=1.2,
            decoration={ markings, mark=between positions 0.1 and 1 step 2.1cm with {\arrow{stealth}} }
        ]
...
\begin{tikzpicture}[scale=1.4,baseline=(current bounding box.center)]
    ...
    \draw[fill=green!80!yellow!45] (-0.7,-1) rectangle (1,1) node[right] {$\Gamma$};
...
Wir erweitern in Abbildung \ref{fig:curve_counting_half} unsere Zählung um halbe Übergänge, d.h. solche Kurvenstücke, welche auf der reellen Achse entweder starten oder enden.
    ...
    \begin{proof}
        ...
        %\begin{enumerate}[i),start=2]
            ...
            \item
                ...
                \end{math}
                Es folgt also $\Index{[a,b]}{p \circ \phi}{q \circ \phi} = -\Index{[a,b]}{p}{q} = \Index{[\phi(a),\phi(b)]}{p}{q}$.

                Sei $\tau$ nun streng monoton fallend, so ist $\hat \tau(x) := \tau \circ \phi$ streng monoton steigend und es gilt
                \begin{math}
                ...
                An diesen Nullstellen $x \in [a,b]$ von $q \circ \tau$, bzw $\tau(x) \in [\tau(a),\tau(b)]$ von $q$ erhalten wir
                \begin{math}
                    \deg\big(q\circ \tau|\Boundary\Gamma_\epsilon(x)\big)
                    = \deg\big(q|\Boundary\Gamma_\epsilon(\tau(x))\big),
                \end{math}
                da $\tau$ streng monoton steigend ist.
            ...
            \item
                Es genügt wieder $a < b$ und $p, q \in \R[x]$ ohne gemeinsame Nullstellen zu betrachten.
                Insbesondere besitzen auch $p + sq$ und $q$ keine gemeinsamen Nullstellen.
...
\begin{algorithm} \label{thm:alg:ci_simple}
    ...
    \begin{algorithmic}[1]
        ...
        \Else
            \State{bestimme $s,r \in \R[x]$ so, dass $p = sq - r$ mit $\deg r < \deg q$} \Comment{z.B. Polynomdivision}
...
\begin{definition} \label{thm:def:pwpoly}
    ...
    Weiter bezeichnen wir für $n \in \R$ die Abbildung $F: \Gamma \to \R^n$ als \emphdef{stückweise polynomiell}, falls die Koordinatenfunktionen $F_i: \Gamma \to \R$, $i = 1, \dotsc, n$ dies sind.
\end{definition}
...
\begin{lemma}[Dreh-Lemma] \label{thm:lem:wn1_rot}
...
\begin{figure}[ht]
    ...
    \pgfplotsset{
        colormap={red}{rgb255(0cm)=(240,160,160); rgb255(1cm)=(240,160,160)},
        colormap={greenyellow}{rgb255(0cm)=(0,128,0); rgb255(1cm)=(180,230,0); rgb255(2cm)=(255,255,0)},
        %colormap={greenyellow2}{rgb255(0cm)=(128,128,0); rgb255(1cm)=(0,255,0)},
    }
    \begin{tikzpicture}[baseline=(current bounding box.center)]
        \begin{axis}[
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[baseline=(current bounding box.center)]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualisierung des Abbildes von $\Gamma_\epsilon$ um eine Durchstoßstelle}
    \label{fig:idx_counting}
\end{figure}
...
\begin{remark}[Zur Zuläsigkeit] \label{thm:rem:idx_badcases}
