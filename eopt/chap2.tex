...
\begin{nt}[“Trust-Region”] \label{2.23}
	...
	%fixme: pred_k sollte skalar sein? pred_k / ared_k oder ared_k / pred_k ? vergleiche auch Trust-Region bei Levenberg-Marquardt später
	\begin{align*}
		\pred_k &:= -s_k \nabla f(x_k), &
...
\begin{kor} \label{2.26}
	\begin{enumerate}[(a)]
		...
		\item
			Sei $f$ strikt konvex und besitze ein eindeutiges Minimum, dann konvergiert die Folge aus Algorithmus 2 % fixme: reference
...
\begin{alg} \label{alg:4}
	\begin{algorithmic}
		...
		\State{$s^- \gets 1$}
		\While{$s^-$ erfüllt nicht \eqref{eq:pw_armijo}}
			\State{$s^- \gets \f {s^-}2$}
		\EndWhile
		\State{$s^+ \gets s^-$}
...
\begin{st} \label{2.38}
	...
	Dann terminiert \ref{alg:4} nach endlich vielen Schritten und liefert eine Schrittweite $s$, die \ref{eq:pw_armijo}, \ref{eq:pw_pw} erfüllt.
	\begin{proof}
		...
		\end{align*}
		Also kann $s^+$ nicht beliebig groß werden und die Schleife terminiert nach endlich vielen Schritten mit einem $s^+$, welches \ref{eq:pw_armijo} verletzt.
		...
		\begin{enumerate}[(i)]
			\item
				In $s^*$ ist \ref{eq:pw_armijo} erfüllt.
			\item
				Für $s \in \R$ mit $g(s) > 0$ ist \ref{eq:pw_armijo} nicht erfüllt.
			\item
				Für $s \in \R$ mit $g(s) \le 0$ ist \ref{eq:pw_armijo} erfüllt.
		\end{enumerate}
		Dann terminiert \ref{alg:4} nach endlich vielen Richtten und liefert eine Schrittweite $s$, die \ref{eq:pw_armijo} und \ref{eq:pw_pw} erfüllt.

		Die Intervallhalbierung sucht nach einem $s^*$ an dem \ref{eq:pw_armijo} noch erfüllt ist und erzeugt dabei eie Folge von $s^-$, die monoton steigend gegen $s^*$ konverrgiert und für alle Folgenglieder gilt $g(s^-) \le 0$.
		...
		\[
			...
			= (\nabla - \gamma) \nabla f(x)^T d,
		\]
		also gilt $\nabla f(x+s^- d)^T d > \eta \nabla f(x)^T d$, was \ref{eq:pw_pw} entspricht.
	\end{proof}
\end{st}
...
\begin{st} \label{2.39}
	...
	\begin{proof}
		...
		\[
			...
			\lim_{L\ni l \to\infty} \f{\nabla f(x_l)^T d_l}{\|d_l\|} = 0
		\]
		Da $f$ nach unten beschränkt ist, erzeugt \ref{alg:4} nach \ref{2.38} für jedes $k \in \N$ eine Schrittweite $s_k > 0$, die \ref{eq:pw_armijo}, \ref{eq:pw_pw} erfüllen.
		Damit gilt
		\[
			f(x_{k+1})
			= f(x_k + s_k d_k)
	...
	\begin{proof}
		\begin{enumerate}[(a)]
			...
			\item
				...
				\begin{align*}
					&\|x_{k+1} - \hat x\| \\
					&\quad \le \|x_k-\hat x\|\|F'(x_k)^{-1}\| \underbrace{\sup_{t\in [0,1]} \| F'(x_k) - F'(\hat x + t(x_k-\hat x)) \|}_{\to 0}.
				\end{align*}
...
\begin{lem} \label{2.51}
	...
	\begin{proof}
		...
		\begin{align*}
			\|F(y) - F(x)\|
			&= \Big\| \int_0^1 F'(x + t(y-x)) \dx[t] (y-x) \Big\| \\
...
Für Konvergenztheorie (für eine Variante dieses Verfahrens) siehe \cite[Satz 21.3]{hanke02}
