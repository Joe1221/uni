...
\begin{st}[Newton-Verfahren, Lokale Konvergenz für Optimierungsprobleme] \label{2.48}
...
\begin{itemize}
	\item
		superlineare Konvergenz (dies rechtfertigt asymptotisch jeden $N$-fachen Aufwand (für beliebig großes, festes $N$) gegenüber einem linear-konvergenten Verfahren)
\end{itemize}
...
\begin{lem}[Sherman-Morrison-Woodberry-Formel] \label{2.55}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			= 0
		\end{align*}
		und $A^{-1} uv^Tv = \|v\|^2 A^{-1} u \neq 0$, also $\dim \ker (A + uv^T) > 0$ und somit $A + uv^T$ nicht invertierbar.
	\end{proof}
\end{lem}
