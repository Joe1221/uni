...
\begin{st} \label{2.38}
	...
	\begin{proof}
		...
		Die Intervallhalbierung sucht nach einem $s^*$ an dem \eqref{eq:pw_armijo} noch erfüllt ist und erzeugt dabei eie Folge von $s^-$, die monoton steigend gegen $s^*$ konverrgiert und für alle Folgenglieder gilt $g(s^-) \le 0$.
	...
	\begin{proof}
		\begin{enumerate}[(a)]
			\item
				...
				Nach \ref{2.48} (a),(b) existierten $\delta, \my > 0$ sodass $\hat x \in \R^n$ der einzige stationäre Punkt in $B_\delta(\hat x)$ ist.
			\item
				...
				Wegen $\nabla f(x_l) \to 0$ (nach \ref{2.57} ist $\hat x$ stationärer Punkt) gilt
				\begin{align*}
					\|x_l - x_{l+1}\|
					&= s_l \|d_l\|
					\le \|d_l\| \\
				...
				\end{align*}
				Sei $\my > 0$ hinreichend klein, dass $\lambda_{\text{min}} (\nabla^2 f(x)) \ge \my$ für alle $x \in B_\delta(\hat x)$ (siehe \ref{2.48}).
