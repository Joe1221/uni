...
\begin{nt}[“Trust-Region”] \label{2.23}
	...
	\end{align*}
	Falls $\f {\ared_k}{\pred_k} \le \gamma$, so wird der Schritt mit verkleinertem $\Delta_k$ wiederholt.
\end{nt}
...
\begin{alg} \label{alg:10}
	\begin{algorithmic}
		...
		\While{$F'(x_k)^T F(x_k) \neq 0$}
			...
			\While{$\eps < \beta_0$} \Comment{Vergrößere $\my$, falls Schritt zu groß}
				\State{$\my \gets 2 \my$}
				\State{$d_k \gets - \big(F'(x_k)^T F'(x_k) + \my I \big)^{-1} F'(x_k)^T F(x_k)$}
				\State{$\eps \gets \frac {\|F(x_k)\|^2 - \|F(x_{k+1})\|^2}{\|F(x_k)\|^2 - \|F(x_k) + F'(x_k) d_k\|^2}$}
			\EndWhile
			\State{$x_{k+1} \gets x_k + d_k$}
			\If{$\eps > \beta_1$} \Comment{Wenn gewünscht, verkleinere $\my$}
