		...
		\begin{enumerate}[(a)]
			...
			\item
				...
				\begin{align*}
					...
					&= F'(x_k)^{-1}\Big( F'(x_k) (x_k-\hat x) - \int_0^1 F'(\hat x + t(x_k-\hat x))(x_k - \hat) \dx[t] \Big)
					&=F'(x_k)^{-1}\int_0^1 \big( F'(x_k) - F'(\hat + t(x_k - \hat x)) \big)(x_k - \hat x) \dx[t],
				\end{align*}
				also
				\[
					\|x_{k+1} - \hat x\|
					\le \|x_k-\hat x\|\|F'(x_k)^{-1}\| \underbrace{\sup_{t\in [0,1} \| F'(x_k) - F'(\hat + t(x_k-\hat x)) \|}_{\to 0}
				\]
...
\begin{st}[Dennis-Moré-Bedingungen]
	...
	\begin{enumerate}[(a)]
		...
		\item
			$\nabla f(x_k) + \nabla^2 f(x_{k+1})(x_{k+1} - x_k) = o(x_{k+1} - x_k) = o(x_{k+1} - x_k)$
	\end{enumerate}
	\begin{proof}
