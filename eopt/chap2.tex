...
\]
$H_k$ erfüllt \ref{eq:qn} nach Übungsaufgabe % fixme:ref
und
\[
	B_{k+1} := H_{k+1}^{-1}
				...
				\begin{enumerate}[(i)]
					...
					\item
						...
						\begin{align*}
							...
							&= \underbrace{(\gamma-\f 12)}_{<0} \underbrace{d_k^T \nabla^2 f(x_k) d_k}_{\ge \my \|d_k\|^2} + \f 12 d_k^T (\nabla f(x_k + sd_k) - \nabla^2 f(x_k)) d_k \\
							&\le \Big(\underbrace{\my (\gamma - \f 12)}_{<0} + \underbrace{\f 12 \|\nabla^2 f(x_k + sd_k) - \nabla^2 f(x_k)\|}_{\to 0} \Big) \|d_k\|^2 \\
							&\le 0
						\end{align*}
