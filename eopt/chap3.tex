...
\coursetimestamp{27}{01}{2014}
		...
		Da $K$ ein Kegel, ist $\lambda y \in K$ für alle $yy \in K, \lambda \ge 0$, also auch $\lambda \nu^T y \le r$ für alle $\lambda \ge 0$.
...
\begin{kor} \label{3.26}
	...
	\begin{proof}
		...
		\ref{3.25} liefert $\not \exists y \in \R^n$ mit $(A B -B)^T y \ge 0$ und $c^T y < 0$.
...
\begin{st} \label{3.27}
	...
	\begin{enumerate}[(a)]
		...
		\item
			$h(\hat x) = 0$,
		\item
			$\lambda \ge 0, g(\hat x) \le 0, \lambda^T g(x) = 0$.
	\end{enumerate}
	\begin{proof}
