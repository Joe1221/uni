...
Es gilt $f^* \circ \d = \d \circ f^*$, deshalb ist $f^*: H^\bullet(N) \to H^\bullet(M)$ wohldefinierter kovarianter Funktor.

\begin{df} \label{6.17}
    Für einen Tensor $\omega$ auf $M$ definiert man den \emphdef{Träger} durch
    \begin{math}
    ...
    Wir definieren die \emphdef[Kohomologiegruppe!kompakter Träger]{$q$-te Kohomologiegruppe in $M$ mit kompaktem Träger}
    \begin{math}
...
\begin{kor}[Poincaré-Lemma] \label{6.19}
    Es gilt
    \begin{math}
