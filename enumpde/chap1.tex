...
\begin{df}[Multiindex und partielle Ableitung] \label{1.1}
	...
	\]
	der Vektor aller partieller Ableitungen der Ordnung $k$ (in beliebiger, fester Reihenfolge).
\end{df}
...
\begin{df}[Räume stetig differenzierbarer Funktionen] \label{1.3}
	...
	Für $n = 1$ schreiben wir auch kurz $C^m(\_\Omega) = C^m(\_\Omega, \R^1)$ und definieren für $u \in C^0(\_\Omega)$ die \emphdef{Supremumsnorm}
	\[
...
\Timestamp{2014-10-17}
			\item
				$(u,v) := \<u,v\>_{L^(\Omega)} := \int_\Omega uv$ ist ein Skalarprodukt auf $L^2(\Omega)$ und $\|u\|_2 = \sqrt{(u,u)}$, also $L^2(\Omega)$ ist vollständig bezüglich einer aus einem Skalarprodukt induzierten Norm, also ein sogenannter \emphdef{Hilbertraum}.
			\item
	...
	\begin{note}
		\begin{itemize}
			...
			\item
				...
				\[
					(\_{\scr L} u)(x) := -\Nabla \cdot (A(x) \cdot \Nabla u(x)) + \Nabla \cdot (b(x) \cdot u(x)) + c(x)u(x),
				\]
...
\begin{df}[Klassifikation] \label{1.15}
	...
	\begin{itemize}
		\item
			\emphdef{elliptisch} in $x$, falls alle Eigenwerte von $A(x)$ positiv sind,
		\item
			\emphdef{parabolisch} in $x$, falls $d-1$ Eigenwerte von $A(x)$ positiv sind, ein Eigenwert Null ist und $\rg([A(x),b(x)]) = d$ gilt,
		\item
			\emphdef{hyperbolisch} in $x$, falls $d-1$ Eigenwerte von $A(x)$ positiv sind und ein Eigenwert negativ ist.
	\end{itemize}
	$\scr L$ ist \emphdef{elliptisch}, \emphdef{parabolisch}, bzw. \emphdef{hyperbolisch}, wenn er es in jedem $x \in \Omega$ ist.
...
\begin{ex} \label{1.16}
	\begin{itemize}
		...
		\item
			...
			\]
			ein Eigenwert von $A$ Null, alle anderen $A$ positiv, $\rg(A, b) = d$.
		\item
	...
	\begin{note}[Anfangs- und Randbedingungen]
		\begin{itemize}
			...
			\item
				...
				Ein AWP wird auch oft \emphdef{Cauchy-Problem} genannt.
		\end{itemize}
	\end{note}
\end{df}
...
\begin{st}[Existenz für AWP der Diffusionsgleichung] \label{1.20}
	Wir definieren die \emphdef{Fundamentallösung} (oder den \emphdef{Wärmeleitungskern}) $\Phi: \Omega_T \to \R$
	\[
...
\begin{st}[Existenz für AWP der Wellengl., d'Alembert'sche Formel, $d = 1$] \label{1.22}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			\partial_t u(x,t) &= \f 12 (c u_0'(x+ct) -cu_0'(x-ct)) + \f 1{2c}( c v_0(x+ct) + cv_0(x-ct)), \\
			\partial_t u(x,0) &= \f 1{2c} (c v_0(x) + c v_0(x)) = v_0(x).
		\end{align*}
		Die PDE ist erfüllt:
		\begin{align*}
	...
	\begin{note}
		\begin{itemize}
			...
			\item
				endliche \emphdef{Ausbreitungsgeschwindigkeit} $c$, denn
...
\begin{st}[Maximumsprinzip für Poisson-RWP] \label{1.24}
...
\begin{nt*}[Motivation für andere Lösungsbegriffe]
	...
	\begin{enumerate}[i)]
		\item
			Unstetiger Quellsterm in der Poisson-Gleichung: falls in $-u'' = f$ für $\Omega := (0,1)$ der Quellterm $f$ unstetig ist, so kann eine Lösung $u$ nicht in $C^2(\_\Omega)$ liegen.
		\item
			Unstetige Koeffizienten im Diffusionsproblem (z.B. geschichtete Materialien) $-\partial_x(a(x) \partial_x u) = 0, x \in \Omega := (0,1)$.
			Falls $a(x)$ unstetig, so kann ein stetiges $u$ mit unstetiger Ableitung eine Lösung sein.
			Betrachte dazu
			\begin{align*}
				a (x) &= \begin{cases}
					1 & x \le \f 12 \\
				...
				\end{cases}.
			\end{align*}
			$u$ ist keine klassische Lösung (keine stetige Ableitung), aber eine sogenannte „schwache Lösung“, in einem geeigneten Sobolev-Raum (siehe \ref{chap:3}).
		\item
...
\begin{nt*}[Numerische Lösungen]
	\begin{itemize}
		\item
			Im Allgemeinen existieren für PDEs (wie bei gewöhnlichen ODEs in Numerik 2 auch) keine Lösungsformeln für eine „exakte Lösung“ $u \in X$, $X$ Banachraum, $\dim(X) = \infty$.

			Daher sind numerische Methoden zur Approximation von $u$ erforderlich.
		\item
			Meistens sind hierzu \emphdef{diskrete Funktionenräume} $X_h \subset X$ gegeben, welche von einem Diskretisierungsparameter $h$ abhängen (z.B. Gitterweite) $n := \dim X_h < \infty$.
		\item
			Die Approximation $u_h \in X_h$ wird durch einen Koeffizientenvektor repräsentiert.
			Sei $X_h = \Span\{\phi_1, \dotsc, \phi_n\}$, wobei $\Set{\phi_i}_{i=1}^n$ Basis, dann lautet der Ansatz $u_h = \sum_{i=1}^n u_i \phi_i$.
			Der Koeffizientenvektor oder DOF-Vektor $\underbar{u}_h = (u_i)_{i=1}^n \in \R^n$ legt die „\emphdef{Freiheitsgrade}“ (engl. “degrees of freedom”, DOF) fest.
		\item
...
\begin{nt*}[Ziele der Numerik und der Numerischen Analysis]
	...
	\begin{itemize}
		...
		\item
			Konvergenzordnung: Maximales $p > 0$, sodass $C > 0$ existiert mit
			\[
				\|u - u_h\| \le C h^p.
			\]
		\item
...
\Timestamp{2014-10-28}
	...
	\begin{itemize}
		\item
			möglichst Dünnbesetztheit (engl. “sparsity”) des LGS $A_h u_h = b_h$, denn $n$ hat in realen Anwendungen Größenordnungen von $10^6$ bis $10^9$.
		\item
