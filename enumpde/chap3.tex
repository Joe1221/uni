...
\begin{st}[Diskretes Problem als LGS] \label{3.50}
	...
	\begin{proof}
		...
		\begin{math}
			a(\sum_{j=1}^{n} u_j \phi_j, w
			= \sum_{i=1}^{n} w_i \underbrace{a(\sum_{j=1}^{n} u_j \phi_j, \psi_i)}_{=l(\psi_i)}
			...
			\item
				...
				\begin{proof}
					\begin{math}
						...
						= \sum_{i,j}^{} a(\sum_{j}^{} v_j \phi_j, \sum_{i}^{} v_i \phi_i )
						= a(v,v)
						> \alpha\|v\|^2
					\end{math}
...
\begin{ex} \label{3.52}
	...
	Bei mäßigem $n$ (z.B. $n \approx 25$) ist dies das \emphdef{Spektralverfahren}.
\end{ex}
...
\begin{ex}[Notwendigkeit von Petrov-Galerkin bei nicht-koerziven Problemen] \label{3.53}
	...
	Mit $V_h = \Span{\Vector{1 & 1}}$, $W_h = V_h$ entsteht ein singuläres System.
	Ansatz: $u_h = u_1 \phi_1$, wobei $\phi_1 = \Vector{1 & 1}$, $u_1 \in \R$ ergibt ein LGS:
	\begin{math}
...
\begin{kor}[Lemma von Céa] \label{3.56}
	Falls $a(\argdot, \argdot)$ stetig und koerziv mit Koerzivitätskonstante $\alpha$, so gilt $\|u - u_h\| \le \frac{\gamma}{\alpha} \inf_{v\in V_h} \|u - v\|$.
	\begin{proof}
...
\begin{note}
	\begin{itemize}
		\item
			$\|u - u_h\|$ ist bei Galerkin-Projektion für $a(\argdot, \argdot)$ koerziv, also höchsten um $h$-unabhängigen Faktor $\frac{\gamma}{\alpha}$ schlechter als die Bestapproximation.
