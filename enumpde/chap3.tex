...
\begin{note}
	\begin{itemize}
		\item
			...
			Es kann passieren und muss verhindert werden, dass $\beta_h \to 0$ für $h \to 0$, da dann die Konvergenzordnung der Bestapproximation oder gar die Konvergenz der Petrov-Galerkin-Approximation (welche später behadnelt wird) verloren geht, was auf ein \emphdef{instabiles Verfahren} führt.
...
\begin{df}[Quadratur auf Referenzelement] \label{3.79}
	...
	\begin{itemize}
		\item
			...
			\begin{math}
				...
				= \sum_{T \in \scr T_h} \int_T (\Nabla \phi_i)^T \phi_{j}
			\end{math}
		...
		\item
			...
			Sei $\hat g: \scr T_h \times \Set{1, \dotsc, n_k} \to \Set{1, \dotsc, m_k}$ derart, dass für globale Basis $\Set{\phi_i}_{i=1}^{m_k}$ von $\P_k(\scr T_h)$ gilt
			\begin{math}
				\hat \phi_{\hat i}(\hat x) = \phi_i(F_T(\hat x))
			\end{math}
			für $i = \hat g(T, \hat i)$, $i \in \Set{1, \dotsc, n_k}$, $T \in \scr T_h$.
			Hierbei ist $\hat i$ der \emphdef[lokaler Index]{lokale Index}, $i$ der \emphdef[globaler Index]{globale Index}.
...
\Timestamp{2014-12-19}
		\item
			...
			\begin{math}
				a_{ij} := \int_{\supp(\phi_i) \cap \supp(\phi_j)} (\Nabla \phi_i)^T \Nabla \phi_j
		...
		\item
			Statt Schleife über $i,j$, führe Schleife über $T \in \scr T_h$ aus, um dann mit einer Schleife über $\hat i, \hat j$ eine lokale Steifigkeitsmatrix $A_{h, T} \in \R^{n_k, n_k}$ zu berechnenn:
			\begin{math}
				A_{h,T} = (\hat a_{\hat i, \hat j, T})_{\hat i, \hat j = 1}^{n_k}
			\end{math}
		\item
			Assembliere die globale Steifigkeitsmatrix $A_h$ durch Addition der lokalen Beiträge in entsprechenden Einträge Einträge von $A_h$:
			\begin{algorithmic}
				\State $A_h \gets 0$
				\For{$T \in \scr T_h$}
		...
		\item
			...
			Eine „einzelner Gitterdurchlauf“ genügt also zur Assemblierung des Systems $A_h, b_h$.
	\end{itemize}
\end{note}
...
\begin{note}[Verallgemeinerungen]
	\begin{itemize}
		...
		\item
			...
			\begin{math}
				(N_1(p), \dotsc, N_{10}(p))
				:= \Big( p(a_0), p(a_1), p(a_2), p(x_S), \partial_{x_1} p(a_0), \dotsc, \partial_{x_1} p(a_2), \partial_{x_2} p(a_0), \dotsc, \partial_{x_2} p(a_2) \Big)
			\end{math}
