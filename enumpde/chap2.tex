...
\begin{ex*}
	Betrachte die FD für das Poisson-RWP, $d = 1$ wie in \ref{2.8}, $h := \f 1{n+1}$, $A_h = \Matrix{2 & -1 &  & \\ -1 & \ddots & & \\ &\ddots & \ddots & -1 \\ & & -1 & 2} \in \R^{n\times n}$.
	...
	Es gilt $\|e\|_\infty = \max_k \f 1, kh(1-kh) \le \f 12 \f 12 ( 1 - \f 12) = \f 18$.
...
\begin{note}[gemischte Ableitung]
	\begin{itemize}
		\item
			In \ref{2.11} wurde $-\sum_{i\neq j}^A a_{ij}(x) \partial_{x_i}^{c,h} \partial_{x_j}^{c,h} u(x)$ gewählt, was zu $\f 12 \Matrix[{a_{12} & 0 & -a_{12} \\ 0 & 0 & 0 \\ -a_{12} & 0 & a_{12}}_*$ führte.
		...
		\item
			...
			\begin{enumerate}[i)]
				\item
					Falls $- a_{12} > 0$:
					\[
						\Matrix[{ a_{12} & -a_{12}e & 0 \\ a_{12} & 2 a_{12} & - a_{12} \\ 0 & - a_{12} & a_{12} }_*
					\]
				\item
					Falls $- a_{12} < 0$:
					\[
