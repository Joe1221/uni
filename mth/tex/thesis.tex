...
\begin{definition} \label{definition:min_stuff}
	...
	\item
	    \emphdef{Gateaux-differenzierbar},
            falls für alle $u, \phi \in B$ das \emphdef{Gateaux-Differential $I': B \times B \to \R$ von $I$ an der Stelle $u$ in Richtung $\phi$}
	    \begin{math}
		I'[u, \phi]
...
\end{math}
%Eine erste Beobachtung ist, dass
%\begin{math}
%    g(x) := \sup_{y} L(x,y)
%    = \begin{cases}
%        f(x) & \text{für $e(x) = 0$}, \\
...
%    \end{cases}
%\end{math}
%Folglich liefert $\hat x := \argmin g(x)$ das gesuchte Minimum von $f$ unter der Nebenbedingung $e(x) = 0$.
%Eine 
...
\begin{algorithm}[Euler Elastica Inpainting]
	...
	\begin{itemize}
	    ...
	    \item
		Ein mögliches Abbruchkriterium wäre $\|u^k - u^{k-1}\| < \epsilon$, wobei die Norm abhängig von der entsprechenden Diskretisierung gewählt wird.
		Allerdings ist die Konvergenzgeschwindigkeit stark abhängig vom gewählten Bild und den eingestellten Parametern, was die Wahl von $\epsilon$ für ein sinnvolles Inpaintingresultat schwierig macht.
...
Für diese speziellen Gitterwahlen werden die Schritte in Algorithmus \ref{algorithm:complete} explizit als lineare Gleichungssysteme oder Updates in den Gitterpunkten dargestellt.

Das Ziel dieser Arbeit bestand in der Implementierung von Algorithmus \ref{algorithm:complete} im Kontext Finiter Element mit Hilfe der C++ Bibliotheken Dune-Fem und Dune-ACFem.
...
Verwendet wurden die GPLv2-lizenzierte „Distributed and Unified Numerics Environment“ Dune\footnote{\url{https://dune-project.org}} samt Core-Module, sowie \texttt{dune-fem} und \texttt{dune-acfem}.
...
In der numerischen Untersuchung des Algorithmus betrachten wir zunächst den vereinfachten TV Inpainting Algorithmus \ref{alg:tv}.
Das hierfür genutzte, synthetische $u^0$ findet sich in \ref{fig:sample}.
...
\begin{figure}[ht]
	...
	\end{tikzpicture}
    \end{subfigure}%
    \caption{In den numerischen Tests vorgegebenes $32\times 32$ Pixel Inpaintingproblem (links) zusammen mit wünschenswertem TV-Inpainting (mittig) und Elastica-Inpainting (rechts)}
\end{figure}

Für TV-Inpainting erwarten wir als Lösung eine diagonale, gerade verlaufende Schwartz-Weiß-Kante, während Elastica-Inpainting eine (je nach Wahl von $\alpha, \beta$) gekrümmte Kante hervorbringen sollte.
