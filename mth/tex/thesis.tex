...
Die Motivation für dieses Vorgehen und die Interpretation des \emphdef{Bildterms} $\Eimg[u]$ und des \emphdef{Datenterms} $\Edat[u]$ liefert das Bayes'sche Prinzip \cite{mumford1994bayesian,chan2005variational} aus der Wahrscheinlichkeitstheorie.
...
\begin{itemize}
    ...
    \item
	Für einen Vektorraum $V$ bezeichnen wir Funktionen $I: V \to \R$ als \emphdef{Funktionale} und schreiben ihr Argument stets in eckigen Klammern: $I[f] \in \R$ für $f \in V$.
    \item
...
Eine Alternative, welche unter wenigen Voraussetzungen (siehe \cite[§4.2.1]{bertsekas1999nonlinear}) konvergiert, ist die \emphdef{Augmented Lagrange Methode}.
...
\begin{figure}[ht]
	...
	\label{fig:num_eeparam1_conv}
    \end{subfigure}%
    \caption{Elastica Inpainting: annähernd konvergentes Verhalten für $r_2, r_4 = 10^4$.}
    \label{fig:num_eeparam_conv}
\end{figure}

Für hohe Parameter $r_2, r_4$ und niedriges $r_1$ zeigt der Algorithmus ein annähernd konvergentes Verhalten (siehe Abbildung \ref{fig:num_eeparam_conv}) und man beobachtet Inpaintingergebnisse wie in den Abbildungen \ref{fig:num_eeparam_png} und \ref{fig:num_eeparam2_png}.
