...
Das Bildmodell Bewertung des Bildresultats auf ganz $\Omega$ (man erinnere sich an das Bayes-Prinzip).
...
Zunächst dürfte mit Blick auf Abbildung \ref{fig:inpainting_non_unique} klar sein, dass die Wahl eines guten Bildmodells keineswegs einfach oder eindeutig ist, außerdem hängt die Bildbewertung oft von der Interpretation des Betrachters ab.

Eine Möglichkeit ist es, zunächst die Levellinien des Bildes und damit insbesondere Kanten zu bewerten, um durch geeignetes Summieren eine Bewertung des Gesamtbildes zu erhalten.
Man spricht hier auch vom \emph{Kurvenmodell} und der \emphdef{Levelset-Methode}.
Kurven und Farbübergänge können so gut bewertet und rekonstruiert werden.

Solche Inpaintingmethoden haben jedoch Schwierigkeiten mit Mustern, die deutlich kleiner sind als der Inpaintingbereich $D$.
Wiederkehrende Strukturen außerhalb $D$ werden nicht innerhalb $D$ fortgesetzt.
Hierfür gibt es das sogenannte “texture based inpainting”, wofür es zahlreiche stochastische Methoden gibt (siehe z.B. \ref{??}).

Das Bildmodell dieser Arbeit macht von der Levelset-Methode gebrauch und nutzt ein Kurvenmodell, dass für jede Levelsetlinie $\gamma$ die Energie
\begin{math}
    \int_{\gamma} \alpha + \beta \kappa^2 \di[t]
\end{math}
minimiert.
Diese Energie modelliert die Biegeenergie eines elastischen Stabes und eine Kurve, die diese Energie (unter geeigneten Randbedingungen) minimiert wird \emphdef{Euler Elastica} genannt.

Im nächsten Kapitel werden wir die Euler Elastica etwas näher untersuchen und die Euler Elastica Energie
\begin{math}
    \Eimg[u] = \int_{\Omega} \alpha + \beta (\nabla \cdot \frac{\nabla u}{\nabla u})^2 \di[x]
\end{math}
für unser Bildmodell herleiten.
