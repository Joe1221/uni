...
\begin{st}[Fehlerschranke Gradientenverfahren] \label{4.15}
	...
	\begin{proof}
		...
		$x^*$ ist Minimum von $\_f$ und der Grenzwert der Grandientenverfahren-Iterierten $(\_x^{(k)})$ genau dann, wenn
		$x^* - x^* = 0$ ist Minimum von $f(x) := \_f(x - x^*)$ und der Grenzwert der Gradienten-Iterierten $(x^{(k)})$ mit $\_x^{(k)} := x^{(k)} - x^*$.

		Für $f(x) = \f 12 \_x^T A \_x$ gilt die Behauptung, also
		\[
