...
\begin{st}[Trigonometrische Interpolation in $\R$]
	\label{1.25}
	Für $n\in \N$ setze $\displaystyle m:= \begin{cases} \f n2 & \text{falls $n$ gerade} \\ \f {n-1}2 & \text{falls $n$ ungerade}\end{cases}$ und $\displaystyle \theta := \begin{cases} 0 & \text{falls $n$ gerade} \\ 1 & \text{falls $n$ ungerade}\end{cases}$.

	Zu $x_k:=\f {2\pi}{n+1}k$ und Daten $\{f_k\}_{k=0}^n\subset \R$ existiert genau eine Funktion
	\[
		t(x) = \f {a_0}2 + \sum_{k=1}^m \Big(a_k \cos(kx) + b_k \sin(kx)\Big) + \f{\theta}2 a_{m+1}\cos((m+1)x)
	\]
