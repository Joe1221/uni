...
\begin{df}[Iterationsverfahren] \label{3.10}
	...
	\begin{note}
		Falls $x^* = \lim_{k\to \infty} x^{(k)}$, so ist $x^*$ Fixpunkt von $\Phi$, d.h. $\Phi(x^*) = x^*$, denn
		\[
			x^* = \lim_{k\to \infty} x^{(k)} = \lim_{k\to \infty} x^{(k+1)} = \lim_{k\to \infty} \Phi(x^{(k)}) = \Phi\Big(\lim_{k\to \infty}\Big) = \Phi(x^*)
		\]
		daher auch wird soein Verfahren auch „\emph{Fixpunktiterationsverfahren}“ genannt.
	\end{note}
\end{df}
	...
	\begin{proof}
		...
		\begin{seg}[„$\Longleftarrow$“]
			...
			\begin{align*}
				...
				= \f \delta 2
			\end{align*}
			und damit $x^{(k+1)} \in B_{\f\delta 2}(x^*)$. Außerdem konvergiert $\Phi(x)$ lokal, da der Kugel immer kleiner wird.
		\end{seg}
		\begin{seg}[„$\Longleftarrow$“]
			\begin{seg}[$p=1$]
			...
			\begin{seg}[$p\ge 2$]
				...
				\[
					a_m := \begin{cases}
						\f{j!\Phi^{(p)}(\xi)}{p!\Phi^{(j)}(x^*)} & m=p \\
						\f{j!\Phi^{(m)}(x^*)}{m!\Phi^{(j)}(x^*)} & j+1 \le m < p
					\end{cases}
				\]
...
\begin{kor}[Verbesserte Newton-Verfahren II] \label{3.16}
	Für $f\in C^{n+2}$ mit $n$-fachre Nullstelle $x^*$ ist das Verfahren
	\[
		x^{(k+1)} := x^{(k)} - n \f {f(x)}{f'(x)}
	\]
	lokal quadratisch konvergent.
	\begin{proof}
		Mit der Notation aus dem vorigen Beweis:
		\begin{align*}
		...
		\begin{enumerate}[i)]
			...
			\item
				...
				\begin{seg}[Existenz]
					...
					$\Phi^*$ ist stetig wegen der Kontraktionseigenschaft, also ist $x^*$ Fixpunkt (siehe Bemerkung \ref{3.10})
				\end{seg}
			\item
			\item
				Betrachte
...
\begin{st}[Fixpunktverfahren mit Relaxation] \label{3.20}
	...
	\begin{proof}
		...
		\begin{align*}
			1 - 2 \omega \gamma + \omega^2 L_F^2 &< 1 \\
	...
	\begin{note}
		\begin{itemize}
			...
			\item
				...
				\[
					\omega_{\text{opt}} := \arg_{\omega} \min c(\omega)
				\]
				Also wenn $c'(\omega_{\text{opt}} = 0$, bzw
				\[
