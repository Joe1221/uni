...
\begin{df}[Zulässige Funktionale]
	\label{2.27}
	Eine lineares Funktional $R : C^k([a,b] \to \R) \to \R$ heißt \emph{zulässig}, wenn es eine Punktauswertung 
	\[
	...
	\]
	oder eine endliche Linearkombination von diesen. \fixme[Tun haben gemacht werden :D]
\end{df}
...
\begin{df}[Bisektionserfahren] \label{3.1}
...
\begin{st}[Konvergenz des Newton-Verfahrens] \label{3.4}
	...
	Dann ist für alle Nullstellen $x^{(k)} \in B_\delta(x^*)$ die Sequenz der Newton-Iterierten $(x^{(k)})_{k\in \N}$ wohldefiniert, $x^{(k)} \in B_\delta(x^*)$ und es gelten folgende Aussagen:
	\begin{enumerate}[i)]
		\item
