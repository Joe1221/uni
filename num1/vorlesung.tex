...
\begin{nt*}
	\begin{itemize}
		...
		\item
			...
			\]
			lassen sich kompakt als Lösung eines LGS schreiben mit Hilfe einer transformierten Vandermondematrix darstellen (siehe Satz \ref{2.10}).
	\end{itemize}
\end{nt*}

\begin{st}
	\label{2.10}
	Die Gewichte $\{\omega_j\}_{j=0}^n$ der geschlossene Newton-Cotes Formel lösen
	\[
		\begin{pmatrix}
			1 & 1 & 1 & \dotsc & 1 \\
...
\begin{nt*}
	\begin{itemize}
		...
		\item
			...
			\]
		\item
			Zusammengesetzt Simpsonregel mit
			\[
		...
		\item
			...
			\begin{table}[h]
				...
				\begin{tabular}{c|c|c}
					...
					3 & $\pm \sqrt{\f 37 + \f 27 \sqrt{\f 65}}, \pm \sqrt{\f 37 - \f 27\sqrt{\f 65}}$
					& $\f {18-\sqrt{30}}{36} $
				\end{tabular}
			\end{table}
	\end{itemize}
...
\begin{nt*}
	...
	\]
	Gauß-Quadraturren auf $[-1,1]$ werden auf Gauß-Legendre-Quadraturen genannt.
\end{nt*}
...
\begin{nt*}[Allgemeiner Integrale]
	Sei $\omega \in C^0(I)$ eine „Gewichtsfunktion“ auf Intervall $I\subset \R$ mit $\int_I \omega(x) dx < \infty$ und $\omega(x) \ge 0$ für alle $x\in I$.
	...
	Definiere analog $\{p_i\}_{i=0}^n$ als orthogonalsystem bezüglich $\<\cdot,\cdot\>_\omega$ von $\P_{n+1}$ wähle die Quadraturpunkte $\{x_i\}_{i=0}^n$ als Nullstellen von $p_{n+1}$, setze $\{\omega_j\}_{j=0}^n$ als gewichtete Integrale der Lagrange-Polynome:
	\[
	...
	\begin{itemize}
		...
		\item
			...
			\]
			mit $p_n = (-1)^n L_n$ („Gauß-Laguerre-Quadratur“).
		\item
			$I=(-\infty,\infty), \omega(x) = e^{-x^2}$, dann bilden die Hermite-Polynome ein Orthogonalsystem:
			\[
		...
		\item
			...
			Dann bilden $Jacobi-Polynome$ ein Orthogonalsystem:
			\[
...
\begin{lem}[Verallgemeinerter Mittelwertsatz]
	...
	\begin{proof}
		...
		\[
			f(x_{\text{min}} = \min_{x\in[a,b]} f(x) , \qquad f(x_{\text{max}} = \max{x\in[a,b]}f(x)
		\]
