...
\begin{st}[Störungs-Aussage]
	...
	\begin{proof}
		Zeige zunächst: $\forall \{\_{f_i}\}\subset \R$ und interpolierende Funktionen $\_p\in V$, gilt:
		\[
...
\begin{lem}[Rekursion für $W_N$]
	\label{1.28}
	Seien $W_N$, $W_{\f N2}$ Matrizen der DFT aus \ref{1.26}. Setze 
	\[
...
\begin{nt*}
	\begin{itemize}
		...
		\item
			...
			Dazu führt man eine FFT durch und setzt $c_k= 0$ für $k$, die zu hohen Frequenzen gehören, anschließend inverse FFT.
	\end{itemize}
\end{nt*}
...
\begin{st}[Existenz und Eindeutigkeit kubischer Splines]
	...
	\begin{proof}
		...
		\end{align*}
		Wir erhalten also ein lineares Gleichungssystem für $n-1$ Unbekannte $M_1,\dotsc,M_{n-1}$:
		\[
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \underbrace{\big( f'(b) - s'(b)\big) s''(b) - \big( f'(a)-s'(a)\big) s''(a)}_{=0 \text{ wegen } \begin{cases}
				...
				\end{cases}}  \\
			&- \sum_{j=1}^n \int_{x_{j-1}}^{x_j} (f-s)'(x))s'''(x) dx \\
		\end{align*}
		Es folgt also
		\begin{align*}
			\int_a^b (f-s)''(x) s''(x) dx = \sum_{j=1}^n \underbrace{\big(-[(f-s)(x)p_j'''(x)]_{x_{j-1}}^{x_j}\big)}_{=0 \text{, da } f(x_j)=s(x_j)} + \underbrace{\int_{x_{j-1}}^{x_j} (f-s)(x)s^{(4)}(x)}_{=0}
		\end{align*}
		Insgesamt gilt dann
		\begin{align*}
...
\begin{st}[Singulärwertzerlegung (SVD)]
	...
	\begin{note}
		\begin{itemize}
			\item
				Falls $A$ reell ist, sind auch $U,V$ reell, also orthogona,l und es gilt
				\[
					A=U\Sigma V^T
				\]
			\item
...
\section{Fixpunktverfahren im $\R^n$}
