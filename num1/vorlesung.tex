...
\begin{df}[Unrestringiertes QP] \label{4.9}
	Wir nennen
	\[
		f(x) = \min_{x\in \R^n} \f 12 x^T A x - b^T x + c
	\]
...
\begin{lem*}[Translationsinvarianz]
	...
	Falls $\_f(\_x) = f(\_x + a)$ und $x^{(0)} = \_x^{(0)}$ für $a \in \R$, so gilt
	\[
		x^{(k)} = \_x^{(k)} + a
		\qquad \land \qquad
		x^* = \_x^* + a
...
\begin{st}[Fehlerschranke Gradientenverfahren] \label{4.15}
	...
	\[
		\|x^{(k)} - x^*\|_A  \le \bigg(1 - \f {2}{\kappa(A) + 1}^k \|x^{(0)} - x^*\|_A
	\]
	...
	\begin{proof}
		...
		Für allgemeine $f(x) = \f 1, x^T Ax - b^Tx +c$ mit $x^* := A^{-1}b$ folgt die Behauptung durch Translationsinvarianz des Gradientenverfahrens (hier nur kurz skizziert):

		\fixme[$\_f$, $f$ und $\_x$, $x$ vertauscht, richtig hinbiegen.]
		$\_x^*$ ist Minimum von $f$ und der Grenzwert der Grandientenverfahren-Iterierten $(\_x^{(k)})$ genau dann, wenn
		$\_x^* = x^* - x^* = 0$ ist Minimum von $\_f(\_x) := \_f(\_x + x^*)$ und der Grenzwert der Gradienten-Iterierten $(x^{(k)})$ mit $\_x^{(k)} := x^{(k)} - x^*$.

		Für $\_f(\_x) = \f 12 \_x^T A \_x$ mit $\_b = 0$ (denn $0= \_x^* = A^{-1}b$) gilt die Behauptung, also
		\[
