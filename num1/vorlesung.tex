	...
	\begin{note}
		\begin{enumerate}
			...
			\item
				...
				Dann gilt
				\begin{align*}
					& p(x) = \sum_{k=0}^n a_k \phi_k(x) \text{ löst die Interpolationsaufgabe}\\[1em]
					\iff \quad & a\in \R^{n+1} \text{ löst } Aa=f \text{ mit } f=(f_i)_{i=0}^n \text{ und } \\
					&\quad A= \begin{pmatrix}\mid & \cdots & \mid \\
...
\begin{table}[ht]
	...
	$x_n$ & $f[x_n]=f_n$ &\\
	\end{tabular}
	\caption{Schema-Darstellung der Rekursion}
\end{table}
		...
		\item
			...
			\begin{table}[ht]
				...
				\begin{tabular}{c|ccc}
					...
					3 & 6
				\end{tabular}
				\caption{Auswertung mit Hilfe des Neville-Schemas}
			\end{table}
	\end{enumerate}
\end{ex*}
...
\begin{table}[ht]
	...
	\end{tabular}
	\caption{Zusammenfassung: Interpolation zu verschiedenen Basen}
\end{table}
...
\begin{ex*}
	...
	\begin{table}[ht]
		...
		\end{tabular}
		\caption{Neville-Schema für die Rekursion der Richardson-Extrapolation}
	\end{table}
\end{ex*}
...
\begin{st}[Gleichmäßige Konvergenz]
	\label{1.12}
	Sei $I=[a,b]$, $f\in C^\infty(I)$ mit $\|f^{(m)}\|_\infty \le M$ für alle $m\in \N$.
...
\begin{note}
	...
	\begin{itemize}
		\item
			...
			denn $f^{(n+1)}(\xi)$ kann ebenso schnell mit $n$ wachsen, wie $(\f {w(x)}{(n+1)!})^{-1}$.
		\item
...
\begin{ex*}[Polynominterpolation, vgl. Deuflhard/Hohmann]~
	...
	\begin{table}[ht]
		...
		\end{tabular}
		\caption{Lebesguekonstanten für äquidistante Stützstellen und für Tschebyscheff-Knoten}
	\end{table}
...
\begin{ex*}
	...
	\begin{table}[ht]
		...
		\begin{tabular}{l|c|c}
			...
			$f_i^{(2)}$ & 12 &
		\end{tabular}
		\caption{Daten für Hermite-Interpolation}
	\end{table}
	...
	\begin{table}[ht]
		...
		\begin{tabular}{l|c|c|c|c|c|c}
			...
			$z_4=2$ & $f_1^{(0)} = 14$ &
		\end{tabular}
		\caption{Neville-Schema für die Rekursion der Newton-Form}
	\end{table}
	Es ergibt sich dann
	\begin{align*}
		p(x)& = -1\cdot 1 + 4\cdot (x-1) + 6(x-1)^2 + 5(x-1)^3 + 1(x-1)^3 (x-2)\\
		&= x^4-2\\
		p'(x) &= 4x^3\\
		p''(x) &= 12x
	\end{align*}
...
\begin{lem}[Reelle und komplexe Fourier-Summe]
	...
	\begin{proof}
		...
		\begin{align*}
			\sum_{k=-m}^m c_ke^{ikx} &= \f {a_0}2 \big(\cos(0x) + i\sin(0x)\big) \\
			& \qquad + \sum_{k=1}^m \f 12 (a_k-ib_k)\big(\cos(kx) +i\sin(kx)\big) \\
			& \qquad + \sum_{k=1}^m \f 12 (a_k+ib_k)\big(\cos(-kx)+i\sin(-kx)\big) \\
	...
	\begin{note}
			...
			Das funktioniert auch umgekehrt für eine gegebene Fouriersumme $\{c_k\}_{k=-m}^m$ mit $c_k=\_{c_{-k}} \in \C$, $c_0\in \R$.
			Die Wahl
			\[
...
\begin{st}[Trigonometrische Interpolation in $\R$]
	...
	\begin{proof}
		...
		\begin{align*}
			a_{m+1} := c_{m+1} + c_{-(m+1)}
			&= c_{m+1} + c_{n+1-(m+1)} \\
			&= c_{m+1} + c_{2(m+1)-(m+1)} = 2 c_{m+1}
		\end{align*}
		also gilt
		\begin{align*}
...
\begin{ex*}
	...
	\begin{table}[ht]
		...
		\begin{tabular}{c|c|c|c}
			...
			$f_j$ & $0$ & $\f 32$ & $\f 32$
		\end{tabular}
		\caption{Daten für trigonometrische Interpolation}
	\end{table}
...
\begin{lem}[Rekursion für $W_N$]
	...
	\[
		W_N := (\omega^{-kj})_{k,j=0}^{N-1} \qquad
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= e^{-2\pi\f {(\f N2 +k)(2j+1)}N} = \underbrace{e^{-i2\pi j}}_{=1} \underbrace{e^{-i\pi}}_{=-1} e^{-i2\pi \f {kj}{\f N2}} \underbrace{e^{-i\f {2\pi}N k}}_{=\omega^{-k}} \\
			&= (-D_{\f N2}W_{\f N2})_{k,j}
		\end{align*}
	\end{proof}
\end{lem}
...
\begin{nt*}[Aufwandsbetrachtung]
		\begin{itemize}
			\item
				...
				\begin{align*}
					C(N) &= \underbrace{2C\left(\f N2\right)}_{\text{Teilprobleme der Größe $\f N2$}} + \underbrace{\f N2}_{\text{Multiplikation mit $D_{\f N2}$}} + \underbrace{\f N2}_{\text{Addition}} + \underbrace{\f N2}_{\text{Substraktion}}\\
					&\le 2 C\left(\f N2\right) + K\cdot N
				\end{align*}
				für ein $K\in \R$.
				Für $N=2^Q$ folgt
				\begin{align*}
...
\begin{st}[Existenz und Eindeutigkeit kubischer Splines]
	...
	\begin{proof}
		...
		\begin{itemize}
			\item
				...
				\begin{align*}
					...
					p_j(x_{j-1}) &= f_{j-1}& &\land& p_j(x_j) &= f_j \\
				\end{align*}
			\item
				Glattheit
				\begin{align*}
					\forall j \in \{1, \dotsc, n-1\}: &&
					p_j'(x_j) &= p_{j+1}'(x_j) & &\land& p_j''(x_j) &= p_{j+1}''(x_j) \\
				\end{align*}
			\item
				Randbedingungen
				\begin{align*}
					p_1''(x_0) &= 0 & &\land& p_n''(x_n) &= 0
				\end{align*}
		\end{itemize}
		Sei $h_j := x_j - x_{j-1}$.
		...
		\end{align*}
		Damit haben wir sowohl die Bedingungen des Zusammenhangs, als auch $p_j''(x_j) = p_{j+1}''(x_j)$ verarbeitet.
...
\begin{st}[Optimalität]
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&\quad= \sum_{j=1}^n \int_{x_{j-1}}^{x_j} (f-p_j)''(x)p_j''(x)) \dx \\
			&\quad= \sum_{j=1}^n \bigg( \Big[(f-p_j)'(x)p_j''(x)\Big]_{x_{j-1}}^{x_j} - \int_{x_{j-1}}^{x_j} (f-p_j)'(x)p_j'''(x) \dx \bigg)\\
			&\quad= \underbrace{\Big( f'(b) - p_n'(b)\Big) p_n''(b) - \Big( f'(a)-p_1'(a)\Big) p_1''(a)}_{ =0 \text{, da } \begin{cases}
				\scriptscriptstyle p_1''(a) = p_n''(b) = 0 & \scriptscriptstyle \text{falls (R1)} \\[0.1em]
				\scriptscriptstyle f'(b) = p_n'(b) = \beta \land f'(a) = p_1'(a) = \alpha  &\scriptscriptstyle\text{falls (R2)} \\
				\scriptscriptstyle f'(a) = f'(b) \land p_1'(a)=p_n'(b) \land p_1''(a)=p_n''(b) & \scriptscriptstyle\text{falls (R3)}
				\end{cases}}  \\
			&\quad\qquad - \sum_{j=1}^n \int_{x_{j-1}}^{x_j} (f-s)'(x))s'''(x) \dx \\
			&\quad= \sum_{j=1}^n \underbrace{\bigg(-\Big[(f-p_j)(x)p_j'''(x)\Big]_{x_{j-1}}^{x_j}\bigg)}_{=0 \text{, da } f(x_j)=p_j(x_j)} + \underbrace{\int_{x_{j-1}}^{x_j} (f-s)(x)s^{(4)}(x)}_{=0}
		\end{align*}
		Insgesamt gilt dann
		\begin{align*}
			\int_a^b |s''(x)|^2 \dx
	...
	\begin{note}
		\begin{itemize}
			...
			\item
				...
				\begin{proof}
					Es gilt $\<v_{k+1}, \dotsc, v_n\> = \ker A^*A$, da Eigenvektoren zum Eigenwert $0$.
...
\begin{st}[Niedrigrang-Matrixbestapproximation]
	...
	\begin{proof}
		...
		\begin{align*}
			...
			= \|Az\|^2
			= \Big\|U\Sigma V^*\sum_{i=1}^n a_i v_i \Big\|^2
			= \|U\Sigma V^* Va \|^2 \\
			&\qquad= \|U\Sigma a\|^2
			\stackrel{\text{Rot. Inv.}}= \|\Sigma a\|^2
			= \l\|\l(\begin{smallmatrix}\sigma_1 a_1 \\ \vdots \\ \sigma_{k'+1}a_{k'+1} \\ 0 \\ \vdots \\ 0\end{smallmatrix}\r)\r\|
			= \sum_{i=1}^{k'+1} \sigma_i^2 |a_i|^2 \\
		...
		\begin{align*}
			\|A-\hat A_k\| &= \|U\Sigma V^* - U\hat \Sigma_k V^*\| = \|U(\Sigma - \hat \Sigma_k) V^*\| \stackrel{\text{Rot. Inv.}}= \|\Sigma - \hat \Sigma_k\|\\
		...
		\begin{enumerate}[i)]
			...
			\item
				...
				\begin{align*}
					...
					&= \bigg(\prod_{l=0}^{m-1} -\Big(x-(a+b-x_l)\Big)\bigg) \cdot \bigg( \f{a+b}2 - x \bigg) \\
					&\qquad \cdot \bigg(\prod_{l=0}^{m-1} -\Big(x-x_l\Big)\bigg) \\
					&= (-1)^{n+1} N(x)
					= -N(x)
				\end{align*}
	...
	\begin{proof}
		\begin{enumerate}[i)]
			\item
				...
				\end{align*}
			\item
				Sei $p_n\in \P_n$ ein Interpolationspolynom zu den Daten $\{(x_i,f(x_i)\}_{i=0}^n$ und
				$q_{n+1}\in \P_{n+1}$ ein \emph{Hermite}"=Interpolationspolynom zu den Daten $\{(x_i,f(x_i)\}_{i=0}^n$, wobei $(x_l,f'(x_l))$ mit $l\in\{0,\dotsc,n\}$ beliebig.
...
\begin{nt*}
	\begin{itemize}
		...
		\item
			...
			\begin{table}[ht]
				\centering
				\caption{Integrationsfehler \fixme[Werte stimmen nicht]}
				\begin{tabular}{l|r}
					n & $|I(f) - I_n(f)|$ \\ \hline
					1 & 0.6667 \\
					2 & 0.6667 \\
	...
	\begin{itemize}
		...
		\item
			...
			\[
				...
				\qquad x_l := a+ lh, \quad l=0,\dotsc,N
			\]
		\item
...
\begin{df}[Orthogonale Polynome]
	...
	\begin{note}
		\begin{itemize}
			...
			\item
				Die orthogonalen Polynome aus \ref{2.17} werden durch das Gram-Schmidt'sche Orthogonalisierungsverfahren, angewandt auf Monome $\{x^i\}_{i\in \N_0}$, konstruiert.
...
\begin{lem}
	...
	\begin{proof}
		\begin{enumerate}[i)]
			...
			\item
				Nach i) ist $\P_{n-1} = \<p_0,\dotsc, p_{n-1}\>$, also reicht es zu zeigen, dass für $i = 0, \dotsc, n-1$ gilt $\<p_n,p_i\>=0$.
				Zeige per Induktion.
			...
			\item
				...
				\begin{seg}[Nullstellen sind reell]
				...
				\begin{seg}[Nullstellen sind paarweise verschieden]
				...
				\begin{seg}[Nullstellen liegen in $(-1,1)$]
...
\begin{st}[Eindeutigkeit der Gauß-Quadratur]
	\label{2.23}
...
\begin{st}[Euler-MacLaurin'sche Summenformel]
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&\quad= \f 12 \big( \phi(1) - \phi(0) \big) - \f 12 B_2(t) \phi'(t) \Big|_0^1 \\
			&\qquad + \underbrace{\f 1{2\cdot 3} B_3(t) \phi''(t) \Big|_0^1}_{=0 \text{ \ref{2.35} ii)}} - \int_0^1 \f 1{3!} B_3(t) \phi^{(3)}(t) \dx[t] \\
			&\quad= \dotsc \\
			&\quad= \f 12 \big( \phi(1) - \phi(0) \big) - \sum_{k=1}^{m-1} \l( \f {B_{2k}(1)}{(2k)!} \phi^{(2k-1)}(1) - \f {B_{2k}(0)}{(2k)!} \phi^{(2k-1)}(0) \r) \\
			&\qquad+ \int_0^1 \f 1{(2m)!} B_{2m}(t) \phi^{(2m)}(t)  \dx[t] \\
			&\quad\stack{\ref{2.35} i)}=\;  \f 12 \big(\phi(1) - \phi(0)\big) - \sum_{k=1}^{m-1} \f {\beta_{2k}}{(2k)!} \big( \phi^{(2k-1)}(1) - \phi^{(2k-1)}(0) \big) \\
			&\qquad+ \int_0^1 \f {B_{2m}(t)}{(2m)!} \phi^{(2m)}(t)  \dx[t]
		\end{align*}
		...
		\begin{align*}
			...
			&= \sum_{j=1}^N \frac{1}{2}(\phi_j(0)+ \phi_j(1))- \sum_{j=1}^N \sum_{k=1}^{m-1} \frac{\beta_{2k}}{(2k)!} ( \phi_j^{(2k-1)} (1)-\phi_j^{(2k-1)}(0)) \\
			&\qquad+\sum_{j=1}^N \int_0^1 \frac{B_{2m}(t)}{(2m)!} \phi_j^{(2m)}(t)\, \dx[t]\\
			&= \sum_{j=1}^N \frac{h}{2} (f(x_j)+f(x_{j-1})) \\
			&\qquad- \sum_{j=1}^N \sum_{k=1}^{m-1} \frac{\beta_{2k}}{(2k)!} h^{2k} (f^{(2k-1)}(x_j)-f^{(2k-1)} (x_{j-1})) \\
			&\qquad + \sum_{j=1}^N \int_0^1 \frac{B_{2m}(t)}{(2m)!} h^{2m+1} \cdot f^{(2m)}(x_{j-1}+ ht)\, \dx[t]\\
			&= I_h(f)- \sum_{k=1}^{m-1} \frac{\beta_{2k}}{(2k)!} h^{2k} (f^{(2k-1)} (b)-f^{(2k-1)}(a)) \\
			&\qquad +h^{2m}\left [h \sum_{j=1}^N \int_0^1 f^{(2m)} (x_{j-1}+ht)\, \frac{B_{2m}(t)}{(2m)!} \dx[t]\right ]
		\end{align*}
		Dies ist die Behauptung falls der Term $[...]$ durch Konstante unabhängig von $h$ beschränkt werden kann. Dies ist der Fall:
		\begin{align*}
			&|h\sum_{j=1}^N \int_0^1 f^{(2m)}(x_{j-1}+t\cdot h) \frac{B_{2m}(t)}{(2m)!} \, \dx[t]| \\
			&\quad\le h \frac{1}{(2m)!} \sum_{j=1}^N ||f^{(2m)}||_\infty \cdot ||B_{2m}||_\infty \\
			&\quad = \underbrace{h\cdot N}_{b-a}\cdot \frac{1}{(2m)!} ||f^{(2m)}||_\infty ||B_{2m}||_\infty
		\end{align*}
		...
		\begin{itemize}
			\item
				...
				\begin{align*}
					...
					&\quad\le \sum_{i=1}^{n_{l}} \Big| I_n(f,K_i^{(l)}) - \int_{K_i^{(l)}} f(x) \dx \Big|
					= E_h(f,\Delta^{(l)}) \\
					&\quad\le \f 1{1-q} \eta^{(l)}
...
\begin{nt*}[Anwendungen]
	\begin{itemize}
		\item
			...
			\[
				|D| = \int_{[0,1]^d} \Ind_D(x) \dx \approx \f 1n \sum_{i=1}^n \Ind_D (x_i)
			\]
			also durch die Monte-Carlo-Approximation für das Integral von $f(x) := \Ind_D(x)$.
		\item
...
\begin{ex*}[Berechnung von $\pi$]
	...
	\[
		|D| = \int_{[0,1]^2} \Ind_D = I
	\]
	Für die Varianz gilt
	\[
		\Var(\Ind_D(X)) = \int_{[0,1]}(f(x) - \tfrac \pi 4)^2 \dx = (1-\tfrac \pi 4) \f \pi 4
	\]
	\begin{figure}[ht]
		\centering
		...
		\item
			...
			\begin{table}[ht]
				...
				\begin{tabular}{c|l|l|l}
					...
					4 & 1.375 & 1.5 & 1.4375
				\end{tabular}
				\caption{Vier Iterationen des Bisektionsverfahrens}
			\end{table}
		\item
		...
		\begin{proof}
			...
			\begin{enumerate}[i)]
				...
				\item
					...
					\begin{align*}
						f(x^{(k)}) &= \underbrace{f(x^{(k-1)}) + f'(x^{(k-1)})\big(x^{(k)}-x^{(k-1)}\big)}_{=0 \text{ (da Newtoniteration)}} + R(x^{(k)},x^{(k-1)}) \\
						&= R(x^{(k)},x^{(k-1)})
					\end{align*}
	...
	\begin{proof}
		...
		\begin{align*}
			\det(\lambda I - M) &= (-1)^{n+n-1}(-1) \det \begin{pmatrix}
				...
				0 &  & -1 & a_{n-2}
			\end{pmatrix} \\
			 &\quad+ (-1)^{n+n} (\lambda + a_{n-1}) \underbrace{\det (\dotsc)}_{=\lambda^{(n-1)}}
		\end{align*}
		Wegen
		\begin{align*}
...
\begin{lem} \label{3.19}
	...
	\]
\fixme[Ungleichung überprüfen:Stimmt das gamma in der Mitte?]
	\begin{proof}
		\begin{seg}[„$\implies$“]
			\begin{align*}
...
\begin{seg}[„Vereinfachtes Newton-Verfahren“]
	...
	\]
	können effizient durch einmalige LR-Zerlegung und mehrfaches Vorwärts-, bzw. Rückwärts"=Einsetzen gelöst werden.
...
\begin{seg}[Intervallschachtelung mit Zwischenpunkten]
	...
	\begin{align*}
		(x_1 - a) &= \tau (x_2 - a) \\
		(1-\tau)(b-a) &= \tau \tau (b-a)
		\; \iff \; 1 - \tau = \tau^2 \;\iff\; \tau = \phi = \f{\sqrt 5 - 1}2 \approx 0.618
	\end{align*}
...
\begin{lem}[Wohldefiniertheit Armijo] \label{4.7}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&\qquad+ \omega_k \Big(\sigma + (1- \sigma)\Big) \nabla f(\xi)^T d^{(k)}\\
			&= f(x^{(k)}) + \omega_k \sigma \nabla f(x^{(k)})^T \cdot d^{(k)} \\
			&\qquad+ \omega_k \bigg(\sigma \Big(\nabla f(\xi) - \nabla f(x^{(k)})\Big)^T d^{(k)} + (1-\sigma)\nabla f(\xi)^T d^{(k)} \bigg)
		\end{align*}
	 ...
	 \begin{note}
		 \begin{itemize}
			...
			\item
				...
				\begin{align*}
					f(x^{(k)} +\omega_k d^{(k)}) &= f(x^{(k)} - c) = c^T x^{(k)} - \|c\|^2 \\
					&\le c^T x^{(k)} - \sigma \|c\|^2 = f(x^{(k)}) + \omega_k \sigma \nabla f(x^{(k)})^T \cdot d^{(k)}
				\end{align*}
				Damit ist
				\[
					x^{(k)} := x^{(0)} - k c
...
\begin{st}[Existenz und Eindeutigkeit des Optimums] \label{4.10}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \f 12 x^T Ax - x^T Ax^* + \f 12 (x^*)^T Ax^* + x^T Ax^* \\
			&\qquad- (x^*)^T Ax^* + (x^* -x)^Tb \\
...
\begin{lem*}[Translationsinvarianz]
	...
	\begin{proof}
		...
		\begin{align*}
			\omega_k &= \argmin_{\omega\in\R} f(x^{(k)} + \omega d^{(k)}) \\
			&= \argmin_{\omega\in \R} f(\_x^{(k)} + a + \omega \_d^{(k)}) \\
			&= \argmin_{\omega\in \R} \_f(\_x^{(k)} + \omega \_d^{(k)})
			= \_\omega_k
		\end{align*}
		also
		\[
...
\begin{ex*}[\normalfont\texttt{demos\textunderscore optimization(5)}]
...
\begin{ex*}[\normalfont\texttt{demos\textunderscore optimization(6)}]
	Sei
	\[
		A = U \begin{pmatrix}
			10 & 0 \\
	...
	\begin{itemize}
		...
		\item
			...
			\begin{table}[ht]
				...
				\end{tabular}
				\caption{Vergleich des Fehlers und der Fehlerschranke beim Gradientenverfahren}
			\end{table}
	\end{itemize}
\end{ex*}
...
\begin{st}[Konvergenz] \label{4.17}
	...
	Das Abstiegsverfahren mit line search für das QP, d.h. für beliebiges $x^{(0)} \in \R^n$ und
	\[
		x^{(k+1)} := x^{(k)} + \omega_k d^{(k)},
...
\begin{st}[Optimierung über verschobenen Krylovraum] \label{4.23}
	...
	\begin{proof}
		...
		\begin{align*}
			&\qquad\qquad\_x^{(k+1)} = \argmin_{x\in x^{(0)} + \scr K_{k+1}} \\
...
\begin{ex*}[\normalfont\texttt{demos\textunderscore optimization(7)}]
...
\begin{ex*}[\normalfont\texttt{demos\textunderscore optimization(8)}]
	Sei $n = 50$ mit
	\[
		A = \begin{pmatrix}
			2 & - 1 & \cdots & 0 \\
...
\begin{ex*}[\normalfont\texttt{demos\textunderscore optimization(9)}]
	Sei
	\[
