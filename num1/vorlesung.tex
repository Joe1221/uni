...
\item Wir nennen einen Fehlerschätzer $\eta^{(l)}$ \emph{zuverlässig}, falls er (bis auf Faktor) obere Schranke für Fehler ist, dass heißt es existiert $\gamma >0$ unabhängig von $l$ und $\Delta^{(l)}$ mit $E_h(f, \Delta^{(l)}\le \gamma \cdot \eta^{(l)}$.
\item Wir nennen $\eta^{(l)}$ \emph{effizient}, falls er den Fehler nicht beliebig überschätzt, dass heißt es existiert $T>0$ unabhängig von $l$ und $\Delta^{(l)}$ mit $\eta^{(l)}\le T \cdot E_n(f, \Delta^{(l)})$.
\end{itemize}
