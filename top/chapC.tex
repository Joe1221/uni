...
\begin{ex}
	...
	(Jede metrische Topologie trennt je zwei Punkte $a \neq b$, die indiskrete Topologie tut das nicht, siehe auch \ref{thm:metric_space_hausdorff}).
\end{ex}
...
\begin{ex}[Koabzählbare Topologie]
	Setze
	\[
		\scr T
...
\begin{nt}
	Seien $X, Y$ beliebige Mengen mit Teilmengen $U, U_i \subset X, V, V_i \subset Y$.
	Es gelten die folgenden Zusammenhänge (leichte Übung):
	\begin{align*}
		f\Big(\bigcup_{i\in I} U_i\Big) &= \bigcup_{i \in I} f(U_i), &
		f^{-1}\Big(\bigcup_{i\in I} V_i\Big) &= \bigcap_{i \in I} f(V_i) \\
		f\Big(\bigcap_{i\in I} U_i\Big) &\subset \bigcap_{i \in I} f(U_i), &
		f^{-1}\Big(\bigcap_{i\in I} V_i\Big) &= \bigcap_{i \in I} f(V_i) \\
		f(X \setminus U) &\neq Y \setminus f(U), &
		f^{-1}(Y \setminus V) &= X \setminus f(V_i).
	\end{align*}
	Man sieht, dass sich die Umkehrfunktionen besser bezüglich den Mengenoperationen verhalten.
	Dies wird im Folgenden für weiterführende Definitionen interessant.
\end{nt}

\begin{df}
	Eine Abbildung $f: (X,\scr T_X) \to (Y, \scr T_Y)$ heißt
	\begin{itemize}
		\item
			\emphdef{stetig}, wenn
			\[
				\forall V \in \scr T_Y : f^{-1}(V) \in \scr T_X.
			\]
		\item
			\emphdef{offen}, wenn
			\[
				\forall U \in \scr T_X : f(U) \in \scr T_Y.
			\]
		\item
			\emphdef{abgeschlossen}, wenn
			\[
				\forall X \setminus A \in \scr T_X : Y \setminus f(A) \in \scr T_Y.
			\]
	\end{itemize}
	\begin{note}
		Diese Eigenschaften sind voneinander unabhängig, d.h. man findet Beispiele, die jeweils unterschiedliche Eigenschaften abdecken.
		In Tabelle \ref{tab:continuous_functions_open_closed} sieht man Beispiele für stetige Funktionen, welche Offen- und Abgeschlossenheit jeweils unterschiedlich erfüllen.
		\begin{table}[ht]
			\centering
			\begin{tabular}{l|ccc}
				$f: \R \to \R$ & stetig & offen & abgeschlossen \\ \hline
				$x \mapsto x$ & ✔ & ✔ & ✔ \\
				$x \mapsto x^2$ & ✔ & ✘ & ✔ \\
				$x \mapsto \arctan(x)$ & ✔ & ✔ & ✘ \\
				$x \mapsto \f 1{1 + x^2}$ & ✔ & ✘ & ✘
			\end{tabular}
			\caption{Beispiele stetiger Funktionen für Offen- und Abgeschlossenheit}
			\label{tab:continuous_functions_open_closed}
		\end{table}
		Entsprechende Beispiele für nicht-stetige Funktionen erhält man durch Modifikation dieser an einem Punkt
	\end{note}
\end{df}
...
\begin{nt}
	...
	\begin{itemize}
		\item
			...
			$f$ ist bijektiv und stetig, aber kein Homöomorphismus.
		\item
			...
			\]
			ist bijektiv, stetig, aber kein Homöomorphismus (es gibt sogar keinen Homöomorphismus zwischen $[0,1[$ und $\S^1$, siehe dazu auch \ref{sphere_non-contractible}).
	\end{itemize}
\end{nt}
...
\begin{ex}
	\begin{itemize}
		...
		\item
			In jedem metrischen Raum $(X,d)$ bilden die Bälle $B(a,\eps)$ um $a \in X$ mit $\eps > 0$ eine Umgebungsbasis von $a$, ebenso $B(a, \f 1k)$ mit $k \in \N_{\ge 1}$ (diese ist sogar abzählbar).
	\end{itemize}
\end{ex}
...
\begin{ex}
	\begin{enumerate}[1.]
		\item
			Jeder metrische Raum erfüllt das erste Abzählbarkeitsaxiom mit Umgebungsbasis $\scr B_i := \Set{ B(x, \f 1k) & k \in \N_{\ge 1} }$.
		\item
...
\begin{prop}
	...
	\[
		\forall V \in \scr B_a \exists m \in \N \forall n \ge m : x_n \in V.
	\]
	% fixme: proof
\end{prop}
...
\begin{ex} \label{thm:metric_space_hausdorff}
...
\begin{st}
	...
	\begin{proof}
		Sei $(x_n)$ eine Folge in $X$ mit $x_n \to a$ und $x_n \to b$.
		Angenommen $a \neq b$, dann existieren mit der Hausdorff-Eigenschaft $U \in \scr U_a, V \in \scr U_b$ mit $U \cap V = \emptyset$, ein Widerspruch zu $x_n \to a, x_n \to b$.
		...
		Angenommen jedoch, für alle $i,j \in \N$ ist $U_i \cap V_j \neq \emptyset$.
...
\begin{st} \label{st:local_bases}
	...
	\begin{enumerate}[(UB1),leftmargin=*]
		\item
			$\scr B_x \neq \emptyset \land \forall U \in \scr B_x : x \in U \subset X$,
		\item
			$\forall U, V \in \scr B_x \exists W \in \scr B_x : W \subset U \cap V$.
	\end{enumerate}

	Erfüllt ein System von Umgebungsbasen $(\scr B_x)_{x\in X}$ die Bedingungen (UB1-2), dann wird die Topologie
	\[
		\scr T_{\scr B_X} := \Big\{ O \subset X : \text{Zu jedem $x\in O$ existiert ein $U_x \in \scr B_x$ mit $U_x \subset O$} \Big\}
	\]
	auf $X$ induziert und
	$\scr B_x$ ist für jedes $x \in X$ eine Umgebungsbasis von $x$ im Raum $(X, \scr T_{\scr B_X})$.
	\begin{proof}
		$\scr T_{\scr B_X}$ ist eine Topologie:
		\begin{enumerate}[(O1),leftmargin=*]
			\item
				folgt aus (UB1).
			\item
				folgt aus (UB2):
				Für $U, V \in \scr T_{\scr B_X}$ ist $W := U \cap V \in \scr T_{\scr B_X}$ zu zeigen.
				Zu $x \in W = U \cap V$ existieren $U_x \in \scr B_x$ mit $U_x \subset U$ und $V_x \in \scr B_x$ mit $V_x \subset V$.
				Mit (UB2) existieren $W_x \in \scr B_x$ mit $W_x \subset U_x \cap V_x$:
				\[
					x \in W_x \subset U_x \cap V_x \subset U \cap V = W.
				\]
				Somit gilt $W \in \scr T_{\scr B_X}$.
			\item
...
\[
	U(f; J \cup J', \min\{\eps, \eps'\}) \subset U(f; J, \eps) \cap U(f, J', \eps')
\]
Die Umgebungsbasis $\scr B_f := \Set{ U(f, J, \eps & J \subset X \text{ endlich}, \eps > 0}$. definiert eine Topologie $\scr T_{\text{pw}}$ (“pointwise”) gemäß \ref{st:local_bases}.

Mit dieser Topologie gilt punktweise Konvergenz $f_n \to f$ genau dann, wenn $f_n \xrightarrow{\scr T_{\text{pw}}} f$.
%fixme: proof?
...
\begin{st} \label{st:pointwise_hausdorff_metrisable}
	...
	\begin{proof}
		...
		dann existiert für jede Umgebung $U_n$ eine endliche Menge $J_n \subset \R$ und $\eps_n > 0$ mit $U(0; J_n, \eps_n) \subset U_n$.
		Die Menge $J = \bigcup_{k\in\N} J_k \subset \R$ ist abzählbar, also $\R \setminus J \neq \emptyset$.
		Für $x_0 \in \R \setminus J$ enthält die Umgebung $U(0; \{x_0\}, 1)$ keine der Umgebungen $U_n$, denn für jedes $n \in \N$ ist mit der Wahl
		\[
			f(x) := \begin{cases}
				0 & x \in J \\
				2 & x \in \R \setminus J
...
\[
	U(f; K \cup K', \min\{\eps, \eps'\}) \subset
	U(f;K,\eps) \cap U(f;K',\eps')
\]
und erzeugt damit die Topologie $\scr T_{\text{kpkt}}$.

\begin{st}
	$f_n$ konvergiert gegen $f$ auf jedem Kompaktum genau dann, wenn $f_n \to f$ bezüglich $\scr T_{\text{kpkt}}$ konvergiert.
\end{st}
...
\begin{st}[Charakterisierung des Abschlusses durch Folgen]
	...
	Wenn es eine Folge $(a_n)_{n\in\N}$ aus $A$ gibt mit $a_n \to x$ in $(X, \scr T)$, dann gilt $x \in \_A$.
...
\begin{df}
	Eine Teilmenge $M \subset X$ heißt
	\begin{itemize}
		\item
			\emph{dicht} in $(X, \scr T)$, wenn $\_M = X$,
		\item
			\emph{diskret} in $(X, \scr T)$, wenn jeder Punkt $x \in M$ isoliert ist.
	\end{itemize}
\end{df}
...
\begin{st}
	...
	\begin{proof}
		%fixme: erstes abzählbarkeitsaxiom, ref?
...
\begin{ex}
	\begin{itemize}
		...
		\item
			Die Teilraumtopologie $\scr T_A$ einer diskreten Menge $A \subset X$ bildet die \emphdef{diskrete Topologie} auf $A$.
	\end{itemize}
\end{ex}
...
%fixme: Dies ist die Universelle Abbildungseigenschaft !!! (genial) bei Summentopologie und Produkttopologie. Extra Sätze draus machen und von hier aus verweisen.
\begin{st}
