...
\begin{st}
	...
	\[
		|\<u,v\>|^2 \le \<u,u\> \<v,v\>.
	\]
	Hieraus folgt für die \emphdef{Norm} $|u| := \sqrt{\<u,u\>}$
...
\subsection{Umgebungen, Offen- und Abgeschlossenheit}
...
\begin{prop} \label{thm:metric_space_closed_subset_convergent_sequences}
...
\begin{conv}
	...
	\end{align*}
	In \coursehref{Blatt01.pdf}{Übungsaufgabe Blatt1, S4.1} wird gezeigt, dass es sich hierbei um $n$-, bzw. $n-1$-dimensionale Mannigfaltigkeiten handelt, siehe auch Kapitel \ref{sec:manifolds}.
\end{conv}
...
\begin{st}
	...
	\begin{enumerate}[1)]
		...
		\item
			$f$ ist \emphdef{folgenstetig}, d.h. aus $x_n \to a$ in $X$ folgt $f(x_n) \to f(a)$ in $Y$,
		\item
...
\begin{st}[Zwischenwertsatz] \label{st:intermediate_value_theorem}
	Jede stetige Funktion $f: X := [a,b] \to \R$ hat die \emphdef{Zwischenwerteigenschaft} (siehe auch \ref{df:intermediate_value_property}): zu $y \in \R$ mit $f(a) \le y \le f(b)$ existiert $x \in \R$ mit $x \in [a,b]$ mit $f(x) = y$.
	\begin{note}
...
\subsection{Äquivalenz von Metriken}
...
\begin{df}
	Ein metrischer Raum $(X,d)$ heißt \emph{totalbeschränkt}, wenn zu jedem $\eps > 0$ eine endliche viele Punkte $a_1, \dotsc, a_n \in X$ existieren mit $X = B(a_n,\eps) \cup \dotsc \cup B(a_n,\eps)$.
\end{df}
...
\begin{st}[Charakterisierung kompakter metrischer Räume]
	...
	\begin{proof}
		...
		\begin{seg}{$(7) \implies (4)$}
			...
			Wir definieren induktiv die Folgen $(x_k^{(n)})_{k\in\N}$ für alle $n \in \N_{\ge 1}$.
			Nach Voraussetzung existieren $a_1, \dotsc, a_m \in X$ mit $X = \bigcup_{k=1}^m B(a_n, 2^{-n})$.
			Mindestens eines der $B(a_n, 2^{-n})$ enthält unendlich viele Folgenglieder.
