...
\begin{df}[Skalarprodukt]
	...
	\begin{note}
		Die Eigenschaft (S0) ist redundant, da sie aus (S1) und (S3) folgt.
	\end{note}
\end{df}

\begin{df}
	Sei $\K \in \{\R, \C\}$.
	Auf $V := \K^n$ definiert $\<\argdot, \argdot\> : V \times V \to \K$ mit
	\[
...
\begin{ex}
	...
	\end{align*}
	Aus $\|f\|_p = 0$ folgt nur $f=0$ fast überall, also ist $\|f\|_p$ auf $\scr L^p(\Omega)$ noch keine Norm.
	Setze
	\begin{align*}
...
\begin{ex}
	Der Euklidische Abstand auf $X := \R^n$,
	\[
...
\begin{ex} \label{ex:discrete_metric}
	Auf jeder Menge $X$ lässt sich die \emph{diskrete Metrik},
	\[
		d(x,y) = \begin{cases}
			0 & x=y \\
			1 & x\neq y
	...
	\]
	definieren.
	Wir nennen dann $(X, d)$ einen \emph{diskreten metrischen Raum}.
\end{ex}
...
\begin{df}
	...
	\begin{note}
		...
		Insbesondere ist also jede isometrische Einbettung ein Homöomorphismus $X \homto f(X)$ gemäß \ref{df:metric_space_homeomorphism} und \ref{df:homeomorphism}.
	\end{note}
\end{df}
...
\begin{st}[Zwischenwertsatz] \label{st:intermediate_value_theorem}
	Jede stetige Funktion $f: X := [a,b] \to \R$ hat die Zwischenwerteigenschaft (siehe auch \ref{df:intermediate_value_property}): zu $y \in \R$ mit $f(a) \le y \le f(b)$ existiert $x \in \R$ mit $x \in [a,b]$ mit $f(x) = y$.
	\begin{note}
		Die Vollständigkeit von $\R$ ist hierfür wesentlich:
		die stetige Funktion $f: \Q \to \Q$ mit $f(x) = x^2 - 2$ hat nicht die ZWE, da $f(1) = - 1 < 0$ und $f(2) = 2 > 0$, aber kein $x \in \Q$ mit $f(x) = 0$ existiert.
	\end{note}
	\begin{proof}
