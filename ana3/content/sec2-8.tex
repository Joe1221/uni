...
\begin{theorem}[Definition] \label{thm:8.6}
  ...
  \end{align*}
  %
  mit $\tilde \phi_j(y_1,..., y_{n-1}):=\phi_j(0, \phi_1,..., \phi_{k-1})$ und $\tilde U_j:= U_j \cap \delta S=\tilde \phi_j(K_1^{(k-1)}(0))$ ein orientierter Atlas von $\delta S$. Die so definierte Orientierung heißt  \acct{verträglich} zur Orientierung von $S$.
