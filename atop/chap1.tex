...
\begin{df}
    ...
    \begin{itemize}
        ...
        \item
            ...
            \end{math}
    \end{itemize}
    Zwei Wege $\alpha, \alpha' \in P(X,a,b)$ heißen \emphdef{äquivalent}, genauer \emphdef{homotop bei festen Endpunkten}, wenn eine stetige Abbildung $H: [0,1] \times [0,1] \to X$, $H_s(t) := H(s,t)$ existiert mit $H_0 := \alpha$, $H_1 = \alpha'$, sowie $H_s(0) = a$, $H_s(1) = b$ für alle $s,t \in [0,1]$.
...
\section{Offene Mengen \texorpdfstring{$X \subset \R^n$}{X ⊆ ℝⁿ} und polygonale Fundamentalgruppe}
...
\end{math}
Damit ist $(A^*, \circ, e)$ ein Monoid (Assoziativität + Einselement), genannt das \emphdef[freies Monoid]{freie Monoid} über $A$.
...
\begin{df}
    Sei $S$ eine Menge von \emphdef{Erzeugern}.
    Zu $S$ wählen wir das Alphabet
    \begin{math}
        A := S \times \Set{\pm} = \Set{s^+, s^- & s \in S}
    \end{math}
    Sei $R \subset A^*$ eine Menge von \emphdef{Relationen}.
    Wir setzen
    \begin{math}
    ...
    \end{math}
    Die durch $(S, R)$ \emphdef{präsentierte Gruppe} ist nun $\Gen{S & R} := \GenMonoid{A & K} = A^* / K$.
\end{df}
...
\begin{math}
    \Set{\emptyset, \Set{a}, \dotsc, \Set{g}, \Set{a,b}, \dotsc, \Set{f,g}, \Set{c,e,f}, \Set{c,e,g}, \Set{c,f,g}, \Set{e,f,g}, \Set{c,e,f,g}}.
\end{math}
...
\begin{df}
    ...
    \begin{note}
        ...
        \end{math}
        Man nennt $\R^{(\Omega)}$ den \emphdef{freier Vektorraum} über $\Omega$“.
    \end{note}
\end{df}
...
\begin{df}
    ...
    \begin{note}
        ...
        \begin{math}
            \Omega &:= \Set{a,b} \cup \N, \\
            K &:= \Set{\emptyset} \cup \binom{\Omega}{1} \cup \Set{\Set{k, k+1} & k \in \N}
        \end{math}
...
Fordere zunächst (unnötig stark)
\begin{itemize}
    \item
    ...
    \begin{proof}[Skizze]
        \begin{enumerate}[1)]
            \item
                ...
                Es existiert eine Lebesgue-Zahl $\frac{1}{n}$, $n \in \N$, sodass $\omega([\frac{k-1}{n}, \frac{k}{n}]) \subset U_{i_k}$ für $k = 1, \dotsc, n$.
                ...
                Definiere $\gamma$ als den zur Indexmenge $i_1, \dotsc, i_n$ natürlich zugehörigen Weg mit eingefügten $\beta_k$.
...
Setze $\dim(S, C) := |S| - 1$.
Definiere die Relation $(S, C) \to (T, D)$ durch $T \subset S$ und $D \supset C$.

Skizze: $U_1, U_2$, $U_1$ mit zwei Komponenten $C, C'$, mit jeweils $3$ Schnitten $D_1, D_2, D_3$ und einem Schnitt $D_4$ mit $U_2$, $U_2$ einfach zusammenhängend.
\begin{math}
    \begin{tikzcd}
    ...
    \end{tikzcd}
\end{math}
Damit ist $I = N^\circ(\scr U)$ mit der Relation $\to$ ein Poset (“partially ordered set”), d.h. reflexiv ($i \to i$) und transitiv ($i \to j \to k \implies i \to k$).
...
% Vermutlich für den allgemeinen Fall von $N^\circ(\scr U)$ Poset, hier aber nicht nötig:
