...
\begin{center}
	...
	\small\url{http://www.mathematik.uni-stuttgart.de/~harrach/lehre/Numerik2.pdf}
\end{center}
...
Zwei Populationen: $y_1(t)$ (Beutetiere) und $y_2(t)$ (Räuber):
\begin{align*}
...
\end{align*}
Es folgt, dass $U_L(t)=C L \ddot U_C(t)$ und $U_R(t)=R C \dot U_C(t)$.
...
\item Elektrische Schaltkreise: Anfängliche Strom- und Spannungswerte am Kondensator $U_C(0)$ und $\dot U_C(0)$
\end{itemize}
...
\[
f:\ \R\times\R^{d}\to \R^d, \quad f(x,y)=\left( \begin{array}{c} f_1(x,y_1,\ldots,y_d)\\ \vdots \\ f_d(x,y_1,\ldots,y_d) \end{array} \right).
\]
...
\begin{align*}
f:\ [x_0,x_\mathrm{end}]\times \R^{d}&\to \R^d\\
(x,y)&\mapsto f(x,y)
\end{align*}
gelte
\begin{enumerate}[(a)]
...
\]
Dann besitzt $\Phi$ genau einen \emph{Fixpunkt} $\hat x$ (d.h. $\hat x\in K$ und $\Phi(\hat x)=\hat x$).
...
konvergiert. Sei also $(y^ {(n)})_{n\in \N}\subset C([x_0,x_\mathrm{end}])^d$ eine Cauchy-Folge bzgl. $\norm{\cdot}_\infty$, d.h. für jedes $\epsilon>0$ existiert $N\in \N$, so dass
\[
\epsilon>  \norm{y^{(n)}-y^{(m)}}_\infty=  \max_{x\in [x_0,x_\mathrm{end}]}  \norm{y^{(n)}(x)-y^{(m)}(x)} \quad \forall n,m\geq N.
\]
...
\[
\norm{y}_w:= \max_{x\in [x_0,x_\mathrm{end}]} \big( w(x) \norm{y(x)}\big)
\]
ein Banachraum.
\end{korollar}
\begin{beweis}
...
\]
Mit $w(x)=e^{- \frac L2 (x-x_0)}$ ist also $\Phi$ eine Kontraktion (mit Kontraktionskonstante $1/2$) 
...
Dann kann man zeigen, dass für jeden Anfangswert $y_0\in Q$, ein nichtleeres Teilintervall $[x_0,x_1] \subset [x_0,x_{\text{end}}]$ existiert auf dem genau eine Lösung existiert.
...
\begin{bemerkung}
\begin{enumerate}[(a)]
\item Auch wenn $f$ nur lokal Lipschitz-stetig, gilt das Stabilitätsresultat in Satz \ref{thm:stability} trotzdem noch dort, wo die eindeutige Existenz der Lösungen gesichert ist.
...
Außerdem kann man zeigen, dass dann die Lösung des AWP ebenfalls unendlich oft stetig 
differenzierbar ist.
\end{enumerate}
\end{bemerkung}
...
$(x,y)\in \R^2$ zeichnen wir einen Richtungspfeil mit Steigung $f(x,y)$, z.B. den Vektor $(1,f(x,y))^T$. Eine Funktion löst die DGL genau dann, wenn an jedem
...
\]
auf dem Intervall $x\in [a,b]$, $a:=x_0$. Wir diskretisieren das Intervall durch $n+1$ äquidistante Punkte
\[
a=x_0 < x_1 < \ldots < x_n=b,
\]
unter Verwendung der \emph{Schrittweite}
...
Die einfachste Möglichkeit ist die \emph{explizite Eulermethode}, bei der wie die Steigung im aktuellen
Punkt verwenden, um die Approximation im nächsten Punkt zu berechnen:\footnote{Hier und im Folgenden bezeichen wir mit $y_0, y_1,\ldots, y_n\in \R^d$ $d$-dimensionale Vektoren und nicht die Einträge eines Vektors $y$.}
...
Bei diesem \emph{impliziten Euler-Verfahren} müssen wir also für jedes $y_i\in \R^d$, $i=1,\ldots,n$,
...
Die anschauliche Idee, eine stückweise lineare Funktion ins Richtungsfeld zu zeichnen, führt auf 
...
\begin{align*}
\norm{f(x,y)-f(x,z)}
&=\|g(1)-g(0)\| \qquad g(t) := f(x,y+t(z-y)) \\
&=\norm{\int_0^1 g'(t) \dx[t]} \qquad g'(t) = f_y(x,y+t(z-y))(z-y) \\
&=\norm{\int_0^1 f_y(x,y+t(z-y)) (z-y) \dx[t]} \\
&\leq L \norm{y-z}
\end{align*}
...
Auf diese globale Zusatzvoraussetzung kann verzichtet werden, wenn die Lösbarkeit sichergestellt ist und die Approximationen an die Lösung beschränkt bleiben (z.B. innerhalb eines beschränkten Gebiets).
...
Im Folgenden betrachten wir nur die Einschrittmethoden (alle in Abschnitt \ref{Sect:NumODE_Motivation} sind Einschrittmethoden).
...
\]
%
\item Besitzt die Methode Konsistenzordnung $p$, so gilt für alle $0<h<h_0$
\[
\max_{i=1,\ldots,n} \norm{y_i-y(x_i)}\leq \frac{e^{L(b-a)}-1}{L} C h^p,
\]
...
\begin{align*}
\norm{y_2-y(x_2)} &\leq \norm{y_2-z(x_2)} + \norm{z(x_2)-y(x_2)}\\
&\leq  C h^{p+1} + e^{Lh} \underbrace{\norm{y_1-y(x_1)}}_{\le Ch^{p+1}}\\
&\leq  (1 + e^{Lh}) C h^{p+1}.
\end{align*}
...
&\leq \ldots \leq  \left( 1+ e^{Lh} + e^{2Lh} + \dotsb + e^{(i-1) Lh} \right) C h^{p+1}\\
...
\begin{align*}
\norm{y(x_{i+1}) - y_{i+1}} &= \norm{y(x_{i+1}) - y_i - h f(x_i,y_i)}   = \norm{y(x_{i+1}) - y(x_i) - h f(x_i,y(x_i))} \\ &= \norm{y(x_{i+1}) - y(x_i) - h y'(x_i)} 
\stackrel{(*)}{\leq} \frac{h^2}{2} \max_{\xi\in [x_i,x_{i+1}]} \norm{y''(\xi)}\leq \frac{h^2}{2} C_2
\end{align*}
mit einer (aufgrund unserer Generalvoraussetzung nur von der rechten Seite $f$ und nicht von $x_i, y_i$ abhängigen) Konstante $C_2$. Es ist also mit obiger Konvention
\begin{align*}
...
\textit{Beweis von $(*)$:} 
Für $y(x) = \left( \begin{array}{c} y^1(x)\\  \vdots \\ y^d(x) \end{array} \right) $  und $\forall j \in \{ 1, \dots , d\}$ folgt:
\[  | y^j(x_{i+1}) - y^j(x_i) - h (y^j)'(x_i) | \leq \frac{h^2}{2} \max_{\xi} | (y^j)''(\xi) | \] Damit folgt insgesamt:
\[ \norm{y(x_{i+1}) - y(x_i) - h y'(x_i)}  = \sqrt{\sum_{j=1}^d | y^j(x_{i+1}) - y^j(x_i) - h (y^j)'(x_i) |^2} \leq \frac{h^2}{2} \cdot \max_{\xi} \underbrace{\sqrt{ \sum_j  | (y^j)''(\xi) |^2}}_{ \leq \norm{y''(\xi)}}  \]
%
...
\end{align*}
da $1 \le \frac{1}{1-hL} \leq 2$ für $hL \leq \frac{1}{2}$.
...
\sum_{j=1}^s b_j=1.
\]
Die $c_j$ heißen \emph{Knoten}, $b_j$ \emph{Gewichte} und $s$ heißt \emph{Stufenzahl} des Verfahrens. $\eta_j \approx y(x_0 + c_j h). $

Dieser Ansatz verallgemeinert die Ideen aus Abschnitt \ref{subsect:More_ex_and_im_methods},
indem nun eine gewichtetes Mittel aus $s$ unterschiedlichen Steigungen im Richtungsfeld an 
...
\begin{algorithm}
	\caption{Runge-Kutta Methoden}
	\begin{algorithmic}
		\Require{$a_{jl}, b_j, c_j\in \R$, $j=1,\ldots, s$, $l=1,\ldots,s$ mit
			\[
...
\]
\begin{algorithm}[Runge-Kutta Methoden (Alternative Formulierung)]
       \begin{algorithmic}
               \Require{$a_{jl}, b_j, c_j\in \R$, $j=1,\ldots, s$, $l=1,\ldots,s$ mit
                       \[
                               \sum_{j=1}^s b_j=1 \quad \mbox{ und } \quad \sum_{l=1}^s a_{jl}=c_j.
...
\frac{\partial \Phi_j}{\partial \eta_l} &=  h a_{jl} \frac{\partial}{\partial \eta_l}f\Big(x + c_l h,\eta_l\Big) \in \R^{d\times d}.
\end{align*}
...
\[
	...
	\le C'h \| \eta^{(2)} - \eta^{(1)} \|
\]
und damit folgt die Behauptung aus dem Banachschen Fixpunktsatz.
\end{beweis}
...
\item 
	Das Verfahren hat genau dann (mindestens) Konsistenzordnung $2$, wenn zusätzlich gilt:
\[
\sum_{j=1}^s b_jc_j=\frac{1}{2}.
\]
\item
	Das Verfahren hat genau dann (mindestens) Konsistenzordnung $3$, wenn zusätzlich gilt:
\[
...
\begin{align*}
y(x_{i+1})=y_i+h f+\frac 12 h^2 f' f + \frac 16 h^3 \Big( f^T f'' f + (f')^2 f \Big) + \mathcal{O}(h^4)
\end{align*}
...
\begin{align*}
	u^T f'' v &= \begin{pmatrix}
		...
		u^T H_{f_1}(y_i) v
	\end{pmatrix}
	= \Bigg( \sum_{j,k=1}^d \frac{\partial^2 f_l(y_i)}{\partial y^{(j)}\partial y^{(k)}} u_jv_k \Bigg)_{l=1}^d
\end{align*}
...
Lösung für $x\to\infty$ mit exponentieller Geschwindigkeit gegen Null.
...
qualitativ richtiges Verhalten zeigen für die komplexe skalare \emph{Testgleichung}
\[
...
\end{theorem}
\begin{beweis}
Anwendung der Runge-Kutta Methode auf die Testgleichung liefert das (möglicherweise implizite) Gleichungssystem
\[
...
Es ist jedoch $R(\im)=1/|1-\im|=1/\sqrt{2}< 1$, das Verfahren ist also nicht Isometrie-erhaltend.
\item Die implizite Mittelpunktsformel ist A-stabil (jedoch nicht L-stabil) und Isometrie-erhaltend (siehe Übungsaufgabe 7.3).
\end{enumerate}
\end{beispiel}
...
&= k_j^{(0)}-(I- a_{jj} h J)^{-1} \left( k^{(0)}_j - f\Big(y_i + h \sum_{l=1}^{j-1} a_{jl} k_l +  a_{jj} h k_j^{(0)}\Big) \right)\\
&= (I- a_{jj} h J)^{-1} \left( f\Big(y_i + h \sum_{l=1}^{j-1} a_{jl} k_l +  a_{jj} h k_j^{(0)}\Big) - a_{jj} h J k_j^{(0)} \right)
\end{align*}

Es bleibt noch die Wahl des Startwertes $k_j^{(0)}$ zu klären. Hierzu verwenden wir eine 
...
\begin{align*}
k_j &:=  (1- a_{jj} h \lambda)^{-1} \left( \lambda\Big(y_i + h \sum_{l=1}^{j-1} (a_{jl}+d_{jl}) k_l \Big) - h \lambda \sum_{l=1}^{j-1} d_{jl} k_l \right)\\
...
\]
$y$ und $\hat y$ werden wir in Übungsaufgabe 8.3 (e) zur adaptiven Schrittweitensteuerung kombiniert. 
...
Wir gehen nun wie im Beweis von Satz~\ref{thm:RungeKuttaOrderCond} vor. Nach Übungsaufgabe 3.2 gilt 
...
y_{i+1}=y_i+hk_2,
\]
vergleichen und müssen dazu also $k_2$ bis zur Ordnung $\mathcal{O}(h^2)$ entwickeln. Dazu benötigen wir die Entwicklung von $k_1$.
Aus 
\[
...
Insgesamt ist also
\[
y_{i+1}=y_i+hk_2=y_i+hf+h^2/2\, f f' + \mathcal{O}(h^3)=y(x_{i+1})+\mathcal{O}(h^3),
\]
...
\[
l_k(x)=\prod_{l=i-m+1,\dots,i \atop l\neq k} \frac{x-x_l}{x_k-x_l}, \quad k=i-m+1,\ldots,i
\]
...
\[
\frac{\partial C}{\partial t}(x,t) =   \frac{\partial}{\partial x} \Big( D(x,t) \frac{\partial}{\partial x} C(x,t)\Big) - \frac{\partial}{\partial x} \Big(v(x,t) C(x,t)\Big) - a(x,t)C(x,t) + M(x,t). 
\]
...
Als Randbedingungen können wir z.B.\ die Konzentration $C(0,t)$ und $C(1,t)$
(Dirichlet Randbedingungen) oder den Fluss  $-D(0,t)\frac{\partial C(0,t)}{\partial x}$ und $-D(1,t)\frac{\partial C(1,t)}{\partial x}$ (Neumann Randbedingungen) für alle $t>0$ vorschreiben.
...
\[
L[u](x):=-u''(x) + b(x)u'(x) + c(x)u(x)=f(x) \quad x\in (0,1)
\]
und zwar zuerst mit \emph{homogenen} Dirichlet-Randbedingungen $u(0)=u(1)=0$. 

Es ist naheliegend, das Randwertproblem zu lösen, indem wir die Funktion $u$
z.B. durch ein äquidistantes Gitter $x_i=ih$, $i=0,\ldots n+1$, $h:=1/(n+1)$ diskretisieren. 
Es bezeichne
\[
...
\[
\tilde F = \left( \begin{array}{c} f(x_0)\\ f(x_1)\\ f(x_2)\\ \vdots \\ f(x_{n})\\ f(x_{n+1}) \end{array}\right)
...
Damit ist
\begin{align*}
\tilde F = \left( \begin{array}{c} f(x_0)\\ f(x_1)\\ f(x_2)\\ \vdots \\ f(x_{n})\\ f(x_{n+1}) \end{array}\right)&\approx
...
\end{align*}
Wiederum ergibt sich ein LGS $\tilde F\approx L_h u+b_h$, und wir erwarten dass
\[
u_h:=L_h^{-1} (\tilde F-b_h)\approx u.
\] 
...
\end{lemma}
\begin{beweis}
Der $i$-te Eintrag ($i=1,\ldots,n$, $x_0=0$, $x_{n+1}=0$) von  $L_h U  - F$ ist 
\[
...
konstruieren wir eine Lösung $w$ eines Randwertproblems, für den die zugehörigen Auswertungen $W$ die Ungleichung
\[
L_h W\geq \1
\]
komponentenweise erfüllen. Zusammen mit einer noch zu zeigenden Monotonieeigenschaft von $L_h^{-1}$ folgt
dann
\[
...
\end{lemma}
\begin{beweis}
Betrachte die Folge der Partialsummen $S_N:=\sum_{k=0}^N R^k$. Da $\sum_{k=1}^\infty \|R\|^k$ konvergent und insbesondere Cauchy-Folge ist, gilt $R^k \to 0$ und
\[
\norm{\sum_{k=0}^N R^k - \sum_{k=0}^M R^k}\leq \sum_{k=M+1}^N \norm{R}^k < \epsilon \quad \forall N>M,
\]
womit $(S_N)_{N\in \N}$ eine Cauchy-Folge ist und damit konvergiert. 
Also gilt
\[
(I-R) \sum_{k=0}^N R^k= \sum_{k=0}^N R^k (I-R) = I- R^{k+1}\to I
\]
und der Grenzwert $S:=\lim_{N\to \infty}S_N=\sum_{k=0}^\infty R^k$ ist die Inverse von $(I-R)$.
\end{beweis}
...
\end{lemma}
\begin{beweis}
Wir zerlegen $A=D-N$ in seinen Diagonal- und Nichtdiagonalanteil. Nach Voraussetzung ist $D\geq 0$ und $N\geq 0$. Für $R=D^{-1} N$ gilt offenbar
\[
...
$L_h$ erfüllt außerdem die Voraussetzungen von Lemma~\ref{lemma:wann_M}. Mit Bemerkung~\ref{bem:monotonie}
folgt dann
\[
...
-\sum_{j=1}^n \frac{\partial^2}{\partial x_j^2} u(x)=-\Delta u(x)=f(x).
\end{equation}
...
-\Delta u=0, \qquad u|_{\partial \Omega} = 0.
\]
...
\]
Die Lösung des Dirichlet-Problems hängen also (sofern sie existieren) stetig von den vorgegebenen Dirichlet-Randdaten ab.
%
...
\item folgt aus Anwendung des Maximumsprinzips und des Minimumsprinzips (die zweite Aussage durch Anwendung auf $-\Delta (u_1-u_2) = 0$ für zwei Lösungen $u_1,u_2$).
\item folgt aus Anwendung des Maximums- und Minimumprinzips auf $u_1-u_2$.
...
%
\item Für $\max_{x\in\Omega} u(x) \le 0$ ist die Aussage klar, sei also $\max_{x\in \Omega} u(x) > 0$.
	Angenommen, es existiert ein $y\in \Omega$ mit 
\[
u(y)=\max_{x\in \overline \Omega} u(x)>\max_{x\in \partial \Omega} u(x) > 0
\]
(das Maximum wird also nicht auf dem Rand angenommen).
Dann gilt $-\Delta u(y)=f(y)-c(y)u(y)\leq 0$ und wir erhalten den gleichen Widerspruch wie im Beweis von Satz \ref{satz:maxprinzip} (ii).
\end{enumerate}
...
zu gegebenen Quelltermen $f:\Omega \to \R$, wobei $\Omega=(0,1)^2\subset \R^2$ ein zweidimensionales, offenes Quadrat mit Seitenlänge $1$ sei.
...
negativ sind, und zeilenweise der Diagonaleintrag dominiert. 
...
Sei $M\in \N_0$ und $\alpha_m,x_m \in \R$ ($m=0,\ldots,M$) mit
\[
	\forall m \ge 1, \alpha_m \le 0 \qquad \sum_{m=0}^M \alpha_m\geq 0, \qquad
...
\[
0\le \sum_{m=1}^M \underbrace{\alpha_m}_{< 0} \underbrace{(x_m - x_0)}_{\leq 0}=\sum_{m=0}^M \alpha_m (x_m - x_0)=\underbrace{\sum_{m=0}^M \alpha_m x_m}_{\leq 0} - \underbrace{x_0\sum_{m=0}^M\alpha_m}_{\geq 0}\leq 0
\]
...
Sei $L_h=(a^h_{ij})_{i,j=1}^N\in \R^{N\times N}$ eine (nicht-notwendigerweise strikt) diagonaldominante Matrix mit positiven Diagonal- und nicht-negativen 
Nebendiagonaleinträgen, also
\[
a^h_{ii}\geq \sum_{j\neq i} |a^h_{ij}|  \quad \forall i=1,\ldots,N, \qquad a^h_{ij}\leq 0 \quad \forall j\neq i.
...
\]
Ist $\displaystyle u^h_i\geq \max\{0,\max_{j:\ j\neq i, a_{ij}\neq 0} u^h_j\}$, so 
...
wenn sie injektiv ist,
also nur der Nullvektor das homogenen LGS $L_h U_h=0$ löst. 

Um dies zu zeigen, sei $U_h$ eine solche Lösung. 
...
\]
$\norm{L_h^{-1}}_\infty < C$ für alle $h>0$ gilt. 
\end{theorem}
\begin{beweis}
...
Oft werden wir uns dabei auf den Fall hermitescher, d.h. $A=A^*$, (oder zumindest diagonalisierbarer) Matrizen beschränken. 
...
\[
\sigma(A):=\Big\{ \lambda\in \C \dd \exists x\in \C^n\setminus\{0\} \dd Ax=\lambda x \Big\}.
\]
...
\item Eigenwerte sind genau die Nullstellen des komplexen \emph{charakteristischen Polynoms} $p(\lambda)=\det(A-\lambda I)$ vom Grad $n$.
 ...
 \norm{A}=\sqrt{\rho(A^*A)}.
 \] 
\item Für eine \emph{hermitesche} Matrix $A=A^*$ sind alle Eigenwerte reell und es existiert eine Orthogonalbasis aus Eigenvektoren. Offenbar folgt damit auch
\[
...
\[
\sigma(A)=\sigma(A^T)\subset \bigcup_{i=1}^n K_i, \quad K_i:=\Big\{ \zeta\in \C \dd |\zeta-a_{ii}|\leq \sum_{\substack{j=1\\ j\neq i}}^n |a_{ji}| \Big\}
\]
\end{bemerkung}
...
Die Menge
\begin{align*}
W(A)&=\Big\{ \zeta=\frac{x^* A x}{x^* x} \dd 0\neq x\in \C^n \Big\} \\
&=\Big\{ x^* A x \dd x\in \C^n,\ \norm{x}=1\Big\}\subseteq \C
\end{align*}
...
Nach Lemma~\ref{lemma:eigW} ist $|\frac {x^* A x}{x^* x}| \le \max\{|\lambda_1|, |\lambda_n|\} = \rho(A) = \|A\|$ und damit $|x^* A x| \le \|A\|\|x\|$.
...
aus Eigenvektoren. Es existiert also eine \emph{unitäre} Matrix $X\in \C^{n\times n}$ (d.h. $X^{-1}=X^*$) mit 
\[
A=X \Lambda X^{-1}, \quad \Lambda=\begin{pmatrix} \lambda_1 & 0 & 0\\ 0 & \ddots & 0\\ 0 & 0 & \lambda_n\end{pmatrix}
\]

\begin{theorem}\label{thm:stabEW}
Sei $A\in \C^{n\times n}$ normal und $E\in \C^{n\times n}$ beliebig. Ist $\tilde \lambda$ ein Eigenwert der gestörten Matrix
...
$(x^{(k-1)})^* \tilde x^{(k)}$ konvergiert also gegen den betragsgrößten Eigenwert $\lambda_1$ und $x^{(k)}$ konvergiert bis auf skalare Vielfache gegen einen dazugehörigen Eigenvektor. Die Konvergenzgeschwindigkeit ist linear mit Konvergenzfaktor $|\frac {\lambda_2} {\lambda_1}|$.
\end{theorem}
\begin{beweis}
...
\begin{enumerate}[(a)]
\item \emph{Inverse Iteration:} Sei $A$ invertierbar. Die betragsgrößten Eigenwerte von $A^{-1}$ sind dann  $\frac 1{\lambda_j}$, wobei 
...
\begin{itemize}
\item Für jedes $k=1,\ldots,n$ spannen die ersten $k$ Vektoren jeweils den gleichen Raum auf, also
\[
\mathrm{span} (x^{(1)}_1,\ldots,x^{(1)}_k ) = \mathrm{span} (A x_1,\ldots, A x_k).
\]
\item $x^{(1)}_1$,\ldots, $x^{(1)}_n$ sind orthonormiert, also
\[
(x^{(1)}_j)^*x^{(1)}_k=\delta_{jk}.
\]
\end{itemize}
...
\]
konvergiert. Dies wird (zumindest wenn die Eigenvektoren senkrecht aufeinander stehen) gerade der betragszweitgrößte Eigenwert sein. Enstprechend erwarten wir Konvergenz
...
\[
\begin{pmatrix} x^{(0)}_1 & \ldots & x^{(0)}_n \end{pmatrix}=\begin{pmatrix} 1 &  & 0\\  & \ddots & \\ 0 &  & 1\end{pmatrix}.
\]
