...
\stack{(*)}{\leq} \frac{h^2}{2} \max_{\xi\in [x_i,x_{i+1}]} \norm{y''(\xi)}\leq \frac{h^2}{2} C_2
\end{align*}
...
Nur für $1+h\lambda \geq -1$ (d.h. $h\leq -\f 2\lambda$) zeigen die Approximationen also das korrekte Langzeitverhalten und 
konvergieren gegen Null, und für $h> -\f 1\lambda$ oszillieren die Approximationen.
...
\item Die implizite Mittelpunktsformel ist A-stabil (jedoch nicht L-stabil) und Isometrie-erhaltend (siehe Übungsaufgabe 6.1).
\end{enumerate}
\end{beispiel}
