...
\item 
	Das Verfahren hat genau dann (mindestens) Konsistenzordnung $2$, wenn zusätlich gilt:
\[
\sum_{j=1}^s b_jc_j=\frac{1}{2}
\]
\item
	Das Verfahren hat genau dann (mindestens) Konsistenzordnung $3$, wenn zusätlich gilt:
\[
...
\begin{align*}
y(x_{i+1})=y_i+h f+1/2 h^2 f' f + 1/6 h^3 ( f^T f'' f + (f')^2 f ) + \mathcal{O}(h^4)
\end{align*}
...
= y_i + h \sum_{k=1}^s a_{jk} (f(y_i) + f'(y_i) (\eta_k-y_i) + \mathcal{O}(h^2))\\
&= y_i + h \sum_{k=1}^s a_{jk} (f(y_i) + f'(y_i) \left(h c_k f(y_i) + \mathcal{O}(h^2) \right) + \mathcal{O}(h^2))\\
&= y_i + h c_j f(y_i)
